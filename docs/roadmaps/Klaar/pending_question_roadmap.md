...
**Datum:** 19 juli 2026 (origineel plan) — geverifieerd tegen echte code 29 juli 2026

> **Belangrijke correctie (29 juli 2026):** dit document beschreef zichzelf lange tijd als "Concept, nog niet gepland" / "PLANNING — nog niet gebouwd", maar bij verificatie tegen de echte `intent_router.py` bleek het mechanisme al volledig gebouwd te zijn, inclusief méér dan hier oorspronkelijk beschreven stond (zie hieronder). Dit document was simpelweg niet meer bijgewerkt na de bouw. Les hieruit: bij twijfel over "is dit al gebouwd?" altijd de echte broncode checken, niet enkel op de status-regel van een roadmap-document vertrouwen.
...
| Fase | Omschrijving | ML nodig? | Status |
|---|---|---|---|
| 1 | Klein `PendingQuestion`-state-object (set/is_open/clear, met verval) | ❌ Nee | ✅ Gebouwd (`pending_question.py`) |
| 2 | `intent_router.py`: check op openstaande vraag vóór normale patroonmatching | ❌ Nee | ✅ Gebouwd — `_verwerk_pending_antwoord()`, aangeroepen als allereerste stap vóór alle andere routing |
| 3 | Koppeling met interpretatie van het antwoord bij een openstaande vraag | ❌ Nee (model bestaat al) | ✅ Gebouwd — via `_interpreteer_ja_nee()`, niet via de zware signal_classifier (lichtere, eigen ja/nee-interpretatie bleek voldoende) |
| 4 | `pending_question:answered`-event, per `vraag_type` opgepikt door de juiste luisteraar | ❌ Nee | ✅ Gebouwd — inclusief een eigen `_on_classifier_pending_answered()`-subscriber |
| 5 | Eerste echte toepassing: `session_watcher.py`'s "mag ik storen?"-vraag hierop aansluiten | ❌ Nee | ✅ Gebouwd |
| 6 (niet in origineel plan) | Correctie-flow: "nee ik bedoelde X" wordt EERST gecheckt, vóór de gewone ja/nee-interpretatie (die een lengte-limiet van 4 woorden heeft en een langere correctiezin anders zou missen) | ❌ Nee | ✅ Gebouwd — `_verwerk_correctie()`, toegevoegd 28 juli 2026, niet in dit document voorzien |

**Afhankelijkheden:** geen — alles hierboven staat al in productie in `intent_router.py`. Het systeem bleek zelfs uitgebreider dan hier oorspronkelijk gepland (zie fase 6): een correctie-mechanisme dat niet in dit document voorzien was, maar wel al gebouwd is.
...
**Status:** ✅ VOLLEDIG GEBOUWD EN GEVERIFIEERD (29 juli 2026) — inclusief een extra correctie-flow die niet in dit oorspronkelijke plan stond
**Auteur:** Claude + Kevin
