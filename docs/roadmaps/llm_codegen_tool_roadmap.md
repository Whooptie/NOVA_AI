...
**Depends on:** Anthropic API-key, bestaand `.env`-patroon (zoals OpenWeatherMap-key)
**Gebruikt door:** losse tool/module buiten Nova's EventBus-brein, optioneel getriggerd via Nova
**Datum:** 11 juli 2026
...
2. [Het kernprincipe: wie beslist wanneer](#kernprincipe)
...
**Optionele uitbreiding — woordfrequentie i.p.v. enkel een teller:**
Sla per fallback-event de zelfstandige naamwoorden op (via bestaande `detect_pos`/tokenisatie uit Layer 1). Dan kan Nova zeggen:
...
| Fase | Omschrijving | ML/LLM nodig? | Geschat werk |
|---|---|---|---|
| 1 | `fallback_triggered`-event publiceren + Layer 1/2 laten meetellen | ❌ Nee | Klein |
| 2 | Woordfrequentie binnen fallback-events (i.p.v. enkel totaalteller) | ❌ Nee | Klein-Middel |
| 3 | Losse `codegen_tool.py`: Kevin-getriggerde Anthropic API-call, output naar bestand | ✅ Ja — expliciet, Kevin-getriggerd | Middel |
| 4 | "Live typen"-effect: vertraagde/blokgewijze write naar bestand | ❌ Nee (puur cosmetisch) | Klein |
| 5 | Context-injectielaag: relevante `nova_state.md`/bestanden automatisch meesturen bij Fase 3 | ❌ Nee (bouw is symbolisch, gebruik voedt een LLM) | Groot |
| 6 | Losse `fallback_analyzer.py`: Kevin-getriggerde export + API-call voor patroonanalyse | ✅ Ja — expliciet, Kevin-getriggerd | Middel |
