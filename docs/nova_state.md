...
5. ~~Uitgebreide uitlegmodule per formule~~ ✅ Gebouwd en live getest (5 augustus 2026, zie `nova_changelog.md` #30 voor het volledige verslag). Nieuwe module `modules/math/math_uitleg.py`, nieuw intent `detect_uitleg()` in `intent_router.py` (herkent "leg uit X"/"leg me uit X"/"kan je uitleggen X"/"wat is X"/"hoe werkt X"). Pilot met 3 functies: `newton`/`nulpunt`, `dv_rk4`, `dijkstra` — later uitbreidbaar met hetzelfde patroon (nieuwe sleutel in `UITLEG_TEKSTEN`). Layer 2-koppeling erbij gebouwd: `help_<topic>` en `uitleg_<canonieke_naam>` worden nu apart geteld in `patterns_layer2.json`, met alias-samenvoeging (verschillende manieren om naar dezelfde functie te vragen tellen samen onder één topic-naam). Apart, niet-blokkerend probleem tijdens het testen ontdekt (typewriter/threading race condition bij lange antwoorden) — genoteerd als nieuw punt 25 hieronder.
6. 🔴 Verwijderpad voor kennis (concepts.json) — momenteel strikt monotoon: geen enkele manier om een concept, sense of relatie te verwijderen (enkel remove_preference() bestaat, en dat is voor voorkeuren, niet voor kennis). Extern geïdentificeerd als het meest structurele gat: een fout die ooit bevestigd wordt, kan nooit meer ongedaan gemaakt worden, en is_a_chained zou zo'n fout voor altijd blijven meenemen in redeneringen.
7. 🔴 find_contradictions() een echte aanroeper geven — de functie bestaat en werkt, maar wordt nergens  aangeroepen (1 facade-methode exporteert hem enkel opnieuw, niets roept die facade zelf aan). Hangt samen met punt 11: zelfs met een aanroeper kan een gevonden tegenstrijdigheid nog niet opgelost worden zonder een verwijderpad — beide punten horen dus eigenlijk samen aangepakt te worden.
8. 🟡 Trust state toevoegen aan concepts.json — nu zien een onzekere en een bevestigde bewering er identiek uit eenmaal opgeslagen (de "unknown"- sentinel is een waarde in het definitieveld, geen echte status, en heeft geen tegenhanger voor "afgewezen").
9. 🟡 ConceptStore.save() atomisch maken — het hele concepts.json-bestand wordt nu bij elke wijziging (zelfs één confidence-aanpassing) volledig herschreven, niet-atomisch — een onderbroken schrijfactie (crash/stroomuitval) kan het hele bestand corrupt maken i.p.v. enkel de laatste wijziging. 
10. 🟡 add_relation()'s hardgecodeerde confidence/source rechttrekken — zet nu altijd confidence: 1.0 en source: "user", ongeacht de werkelijke aanroeper, terwijl de auto-extractor elders wél correcte, gevarieerde provenance wegschrijft (bv. "source": "wikipedia"). Precies het veld dat via het confirmation-gate het meest betrouwbaar zou moeten zijn, is nu net het minst informatieve.
11. 🟡 Semantic Fase 8+ (semantic_extension_roadmap.md blijft leidend) — start met Fase 8 (Causal Reasoning), pure symbolisch, geen ML nodig reasoning_engine_ideeen_roadmap.md is een LOSSE ideeënbak zonder eigen bouwvolgorde (niet concurrerend) — idee #5 ("waarom niet"-uitleg) en #1 zijn daaruit de meest voor de hand liggende kandidaten om tussendoor op te pakken
12. 🟢 **Layer 1 — 3 nooit-gebouwde "Advanced queries" uit memory_layer1_roadmap.md** — bij de fase-controle (29 juli 2026) gevonden als enige overgebleven gaten in een verder volledig afgeronde Layer 1:
