...
## 💡 Idee (nog niet ingepland): generieke actie-koppeling per Intent Classifier-categorie

Kevin's observatie (29 juli 2026): bij een bevestigde classifier-vraag (of een toekomstige `≥0.70`-directe match) krijgt enkel `chess` een ECHTE actie (`intent_chess_new`/`intent_chess_board`, via een specifieke `if label == "chess":`-tak in `_voer_classifier_intent_uit()`). De overige 9 categorieën (inclusief het nieuwe `chess_evaluation`) krijgen enkel een neutrale bevestigende tekst ("Oké, dus je wil X. Zeg maar wat je precies bedoelt!") — er gebeurt daarna geen enkele concrete actie, Nova wacht gewoon op een vrij vervolgbericht.

**Concreet gemist geval:** "was die zet nu eigenlijk wel verstandig" → classifier gokt `chess_evaluation` → bevestigd → GEEN `intent_chess_evaluation_query` wordt gepubliceerd, terwijl die actie wél bestaat en exact hetzelfde doet als `detect_chess()`'s vaste-lijst-tak voor dezelfde vraag.

**Waarom dit niet zomaar per categorie gepatcht moet worden:** een aparte `if label == "...":`-tak toevoegen per categorie (zoals nu bij chess) schaalt niet — bij elke nieuwe categorie moet dan opnieuw beslist worden "verdient dit een eigen actie-tak?", wat op termijn een lange, ad-hoc if/elif-ketting in `_voer_classifier_intent_uit()` oplevert.

**Voorstel indien dit ooit gebouwd wordt:** een generiek mechanisme waarbij élke categorie een gekoppelde actie/event kan hebben (bv. een dictionary `label -> event_naam`), zodat een bevestigde classifier-gok gewoon netjes doorstroomt naar hetzelfde soort afhandeling als de bestaande `detect_*()`-matching, i.p.v. hardcoded per-label if-statements. Belangrijk openstaand vraagstuk hierbij: de classifier kent zelf geen sub-informatie (bv. bij `chess` weet ze niet OF het een zet, nieuwe partij of bordweergave is) — dat maakt een generieke koppeling minder triviaal dan het lijkt voor categorieën die wél sub-acties hebben, terwijl het voor "kale" categorieën zoals `chess_evaluation` (één simpel, vast event, geen sub-informatie nodig) net wel meteen zou werken.

**Waarom nu nog niet bouwen:** vereist eerst een apart ontwerpgesprek over hoe dit mechanisme zich verhoudt tot categorieën MET sub-acties (chess) versus categorieën ZONDER (chess_evaluation, en de meeste van de overige 9). Voor nu blijft chess_evaluation dus bij de neutrale bevestigende tekst, net als de andere 8.
...
3. 🟢 **Generieke actie-koppeling per Intent Classifier-categorie** — concept, nog niet ingepland (zie eigen sectie "💡 Idee (nog niet ingepland): generieke actie-koppeling per Intent Classifier-categorie" verderop voor het volledige voorstel en de reden om nu nog niet te bouwen).
...
6. 🟢 conversation_engine.py — rustig-/overprikkeld- observatietakken nog live bevestigen (enkel energiek-tak is tot nu toe getest)
7. 🟡 math.py Fase 3-5 afwerken — substantieel werk,
...
8. 🟡 Semantic Fase 8+ (semantic_extension_roadmap.md blijft leidend) — start met Fase 8 (Causal Reasoning), pure symbolisch, geen ML nodig reasoning_engine_ideeen_roadmap.md is een LOSSE ideeënbak zonder eigen bouwvolgorde (niet concurrerend) — idee #5 ("waarom niet"-uitleg) en #1 zijn daaruit de meest voor de hand liggende kandidaten om tussendoor op te pakken
9. 🟢 **Layer 1 — 3 nooit-gebouwde "Advanced queries" uit memory_layer1_roadmap.md** — bij de fase-controle (29 juli 2026) gevonden als enige overgebleven gaten in een verder volledig afgeronde Layer 1:
...
17. 🟢 **Taal & Redeneerlimieten — 4 losse ideeën** (nieuwe roadmap: `taal_en_redeneerlimieten_roadmap.md`) — ontstaan uit een extern AI-advies over hoe ver Nova's taal/redeneren op te rekken zijn zonder LLM-kern. Losse ideeënbak zonder eigen bouwvolgorde (zelfde opzet als `reasoning_engine_ideeen_roadmap.md`):
