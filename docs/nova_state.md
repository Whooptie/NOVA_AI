...
5. 🟢 **Nieuw idee (2 aug 2026, nog niet gebouwd): uitgebreide uitlegmodule per formule.** Kevin: "ik snap nog steeds maar 10% van al die formules" — het bestaande `modules/help/topics/math.py` (help-overzicht) geeft enkel een korte definitie + voorbeeld-input per functie, geen diepgang. Idee: een nieuw commando (bv. `leg uit kracht` / `uitleg energie_kinetisch`) dat per functie een uitgebreid mini-lesje teruggeeft — de formule stap voor stap opgebouwd, waarom ze wiskundig/natuurkundig klopt, én een concreet dagelijks voorbeeld (niet enkel de korte voorbeeld-input die er al staat). Kevin koos bewust voor combinatie van twee eerder besproken opties: (A) uitgebreide uitleg per functie, en (B) een soort interactieve leermodus-gevoel, niet enkel een statische tekst. Nog te bepalen: waar deze uitleg-teksten wonen (nieuw bestand naast `modules/help/topics/math.py`, of in `math.py` zelf als een `leg_uit()`-achtige functie), hoe `detect_help()`/`intent_router.py` het commando moet herkennen (nieuw intent, of een variant op het bestaande `help <topic>`-patroon), en of alle 60+ functies er een krijgen of enkel de meest gebruikte/moeilijke (bv. Newton-Raphson, RK4, Dijkstra, kansrekening) als eerste stap.
