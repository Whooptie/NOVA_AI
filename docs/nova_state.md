...
| 33 | Een op zichzelf staande, nieuwe vraag over hetzelfde onderwerp ("wat is het weer", zonder enig verwijzend woord) wordt door de referentie-resolutie toch behandeld als een "kale verwijzing" naar een eerdere, andersluidende weer-vraag ("wat wordt het weer **morgen**") zodra die recent genoeg in `last_context` staat. Gevolg: de nieuwe vraag wordt herschreven met de oude tijdsaanduiding erin ("wat is wat wordt het weer morgen weer"), en Nova geeft het weer van **morgen** terug terwijl er naar **vandaag** gevraagd werd — zonder enige foutmelding, dus dit valt niet op tenzij je de twee antwoorden vergelijkt. Ontdekt tijdens de allereerste live-test van Nova in de Docker-container op Unraid (5 september 2026). Vermoedelijke oorzaak: `_is_kale_verwijzing()` in `intent_router.py` herkent "wat is het weer" blijkbaar als kaal genoeg om te herschrijven, zonder te checken of de zin ook prima op zichzelf, zonder herschrijving, een geldige/beantwoordbare vraag is. | intent_router.py (`_is_kale_verwijzing()`) en/of `core/last_context.py` | 🟡 Midden | 🔲 Open — nog te onderzoeken: `_is_kale_verwijzing()` zou een zin die zelf al een compleet, routeerbaar patroon vormt (hier: `detect_weather()`'s eigen trigger, los van tijdsaanduiding) niet als "kaal" mogen bestempelen, ongeacht of er een recent gerelateerd onderwerp in `last_context` staat. |
| 34 | Stockfish (schaak-engine) start niet in de Docker-container: `[CHESS] Kon Stockfish niet starten: [Errno 2] No such file or directory: 'stockfish'`. De Dockerfile installeert Stockfish via `apt-get install stockfish`, en `chess_engine.py`'s `STOCKFISH_PATH`-fallback (`os.getenv("STOCKFISH_PATH", "stockfish")`) verwacht dat `stockfish` daarna gewoon in het systeem-`PATH` staat — dat blijkt in de container niet (meteen) het geval, of het binary staat op een ander pad dan verwacht. Nog niet onderzocht welke van de twee het is. Ontdekt tijdens de allereerste live-test van Nova in de Docker-container op Unraid (5 september 2026), bij het proberen zetten van een schaakzet ("pion naar e4") na het correct tonen van het bord. | chess_engine.py (`STOCKFISH_PATH`-fallback) en/of Dockerfile (apt-get-installatiestap/PATH) | 🟡 Midden | 🔲 Open — nog te onderzoeken: in de container `which stockfish` draaien om het werkelijke installatiepad te vinden, en indien nodig `STOCKFISH_PATH` expliciet als environment variable meegeven in de container-instellingen i.p.v. op de fallback te vertrouwen. |
