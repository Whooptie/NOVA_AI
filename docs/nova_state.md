...
| math.py | 🟡 Fase 1-4 volledig klaar, Fase 5 open | Berekeningen, temperatuurconversie, wiskundige functies, volledige Fase 3, en volledige Fase 4 (punt 10 Symbolische algebra via SymPy — enige plek in math.py met een externe bibliotheek — en punt 11 Fysica-engine: krachten, energie, beweging, projectielbanen, simulaties) — zie math_roadmap.md |
...
4. 🟡 math.py Fase 5 afwerken — Fase 3 én Fase 4 volledig af (2 aug 2026,
   punt 7-11 alle vijf getest in Nova zelf), in volgorde van math_roadmap.md:
   └── Fase 5 + aanvulling: Getaltheorie/CS-algoritmes, precisie/notatie/exacte vormen (16 losse, kleine ideeën, alle 100% symbolisch — let op: faculteit/combinaties/permutaties uit punt 12 zijn al gebouwd in punt 9)
   ...
   **Belangrijke bugfix (2 aug 2026):** preprocess() interpreteerde "3x^2" verkeerd als "(3x)^2"=9x² i.p.v. 3x² — trof alle vrije-expressie-functies (afgeleide, newton, polyeval, extremum, integraal, limiet, dv, en fysica-functies). Gefixt, geregressietest.
