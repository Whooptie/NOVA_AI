...
5. 🔴 Verwijderpad voor kennis (concepts.json) — momenteel strikt monotoon: geen enkele manier om een concept, sense of relatie te verwijderen (enkel remove_preference() bestaat, en dat is voor voorkeuren, niet voor kennis). Extern geïdentificeerd als het meest structurele gat: een fout die ooit bevestigd wordt, kan nooit meer ongedaan gemaakt worden, en is_a_chained zou zo'n fout voor altijd blijven meenemen in redeneringen.
6. 🔴 find_contradictions() een echte aanroeper geven — de functie bestaat en werkt, maar wordt nergens  aangeroepen (1 facade-methode exporteert hem enkel opnieuw, niets roept die facade zelf aan). Hangt samen met punt 11: zelfs met een aanroeper kan een gevonden tegenstrijdigheid nog niet opgelost worden zonder een verwijderpad — beide punten horen dus eigenlijk samen aangepakt te worden.
7. 🟡 Trust state toevoegen aan concepts.json — nu zien een onzekere en een bevestigde bewering er identiek uit eenmaal opgeslagen (de "unknown"- sentinel is een waarde in het definitieveld, geen echte status, en heeft geen tegenhanger voor "afgewezen").
8. 🟡 ConceptStore.save() atomisch maken — het hele concepts.json-bestand wordt nu bij elke wijziging (zelfs één confidence-aanpassing) volledig herschreven, niet-atomisch — een onderbroken schrijfactie (crash/stroomuitval) kan het hele bestand corrupt maken i.p.v. enkel de laatste wijziging.
9. 🟡 add_relation()'s hardgecodeerde confidence/source rechttrekken — zet nu altijd confidence: 1.0 en source: "user", ongeacht de werkelijke aanroeper, terwijl de auto-extractor elders wél correcte, gevarieerde provenance wegschrijft (bv. "source": "wikipedia"). Precies het veld dat via het confirmation-gate het meest betrouwbaar zou moeten zijn, is nu net het minst informatieve.
...
11. 🟢 **Layer 1 — 3 nooit-gebouwde "Advanced queries" uit memory_layer1_roadmap.md** — bij de fase-controle (29 juli 2026) gevonden als enige overgebleven gaten in een verder volledig afgeronde Layer 1:
