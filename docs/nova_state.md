...
| debug_commands.py | ✅ Klaar (nieuw, 24 juli 2026; uitgebreid 26 juli 2026, 6 augustus 2026) | Bundelt alle tijdelijke debug-/testcommando's die voorheen los in`main.py`'s invoerlus stonden (~320 regels, zie changelog). Afwijkende `init_module(event_bus, loader)`-signature (krijgt de loader zelf mee i.p.v. `sem`), handmatig geladen in `module_loader.py` net als `pending_question`/`interruption_tracker`/`response_engine`. `main.py` checkt enkel nog `debug_module.is_debug_command(user_input)` en publiceert bij match het `debug_command`-event. Documentatie via nieuw topic `help debug` (`modules/help/topics/debug.py`). Ondertussen 17 commando's: de oorspronkelijke 14 (24 juli 2026), plus `preferences debug` (26 juli 2026), `associaties`/`associaties <woord>` (26 juli 2026, Layer 1 — nodig geworden om de ruis-bug uit `chat_response`-events op te sporen, zie changelog), `contradicties` (6 augustus 2026, punt 2 — forceert de contradiction_checker-check meteen, toont alles inclusief al-gemelde conflicten zonder de spam-preventie-state te wijzigen), en `bridge <woord1> en/met <woord2>` (9 augustus 2026, Layer 1 — toont find_bridge()'s brugwoorden tussen twee woorden, zie changelog; syntax later aangepast van `<woord1> <woord2>` naar verplicht `en`/`met` ertussen, zie changelog voor de "en als 2e woord"-bugfix). Live getest (24 juli 2026): boot OK, `help debug`, `emergence`, `geheugen gezondheid` werken correct. |
...
6. ✅ ~~**Layer 1 — 2 nog openstaande "Advanced queries" uit memory_layer1_roadmap.md**~~ — **AFGEROND (11 augustus 2026)**, zie `nova_changelog.md`. `get_trending(window_days, top_k)` en `get_positive_words()`/`get_negative_words()` (met `min_score`-drempel) gebouwd, getest (14/14 pytest) en gekoppeld aan nieuwe debugcommando's `trending [<dagen>]` en `sentiment woorden`.
6b. 🟢 **`get_trending()` koppelen aan Nova's echte gesprekslogica — zowel reactief als proactief (Layer 7).** Zelfde situatie als destijds bij `find_bridge()` (zie punt 6b bij `find_bridge()`, ondertussen afgerond): `get_trending()` is nu enkel bereikbaar via het debugcommando, nog niet via een gespreks-intent of via Nova's eigen initiatief. Twee mogelijke, niet-exclusieve invalshoeken, beide nog te ontwerpen:
   ├── **Reactief:** een nieuwe intent zoals "waar leer ik nu over" → `get_trending()`.
   └── **Proactief, via Layer 7 (`emergence_engine.py`):** een 5e insight-type naast de bestaande 4 (`analyze_topic_frequency`/`analyze_timing_pattern`/`analyze_knowledge_density`/`analyze_personality_drift`), bv. `analyze_trending_topic()`. Zou hetzelfde stramien volgen als de bestaande insight-types: eigen `MIN_*`-betrouwbaarheidsdrempel, eigen `LAYER4_DREMPELS`-waarde, eigen sjabloonvariant — de bestaande confidence-gate en timing-gate (`_mag_nu_spreken()`) hoeven niet opnieuw gebouwd te worden. Onderscheid t.o.v. het bestaande `analyze_topic_frequency()`: dat insight toont het sterkste verband over de HELE geschiedenis (verandert nauwelijks), `get_trending()` toont wat RECENT NIEUW is — vult dus een écht gat, geen duplicaat.
...
`C:\Nova_AI\tests\`. Draaien vanuit de project-root: `pytest tests/ -v`. Stand 9 augustus 2026: **237 tests, allemaal groen** (~8s).
