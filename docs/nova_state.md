...
8. 🟢 **Taal & Redeneerlimieten — 4 losse ideeën** (nieuwe roadmap: `taal_en_redeneerlimieten_roadmap.md`) — ontstaan uit een extern AI-advies over hoe ver Nova's taal/redeneren op te rekken zijn zonder LLM-kern. Losse ideeënbak zonder eigen bouwvolgorde (zelfde opzet als `reasoning_engine_ideeen_roadmap.md`):
...
`C:\Nova_AI\tests\`. Draaien vanuit de project-root: `pytest tests/ -v`. Stand 22 augustus 2026: **410 tests, allemaal groen** (~5s).
...
**`intent_router.py` (~3500 regels) en `chat.py`: BEWUST GEEN volledige dekking, expliciet benoemd om een te optimistische indruk te voorkomen.** Getest zijn tot nu toe enkel de stukken die tijdens een sessie zelf gebouwd/gewijzigd werden: `bridge_query` (zie hierboven), de `-1F`-reactivatie-routing, en Bug #37's woordherkenning. NIET getest: de overige `detect_*()`-methodes (`detect_relation_check`, `detect_part_of_check`, `detect_subtypes_query`, `detect_parts_query`, `detect_related_to_check`, `detect_compare_concepts`, `detect_activity`, `detect_preference`/`detect_preference_query`, classifier-fallback-pad, chess/math/weather/time-routing, help/uitleg-routing) en de meeste `on_*()`-handlers in `chat.py` (`on_definition`, `on_relation_check`, `on_part_of_check`, `on_subtypes_query`, `on_parts_query`, `on_related_to_check`, `on_compare_concepts`, `on_parts_with_property`, `on_related_to`, `on_synonym`, `on_antonym`, `on_used_for`, `on_causes`, `on_properties`, `on_meaning`, `on_concept_learned`, `on_wiki_response`, `on_identity_question`, `on_self_architecture_question`). Risico-inschatting nog niet gemaakt — als dit ooit opgepakt wordt, eerst afwegen welke van deze methodes het hoogste stille-regressierisico hebben (zelfde aanpak als de rest van dit hoofdstuk).
