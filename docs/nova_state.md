...
- `math` — was oorspronkelijk uitgesloten omdat de module zelf nog niet af was; math.py is inmiddels volledig af (2 aug 2026, Fase 1 t/m 5). Of `math` alsnog in dit register hoort, staat als apart, nog te beslissen werkpunt in "Volgende stappen" hieronder.
...
- Vervolgstap (nog niet gebouwd): contextuele oplos-suggestie na `leg uit X` — zie "Volgende stappen" hieronder.
...
23. 🟢 **Contextuele oplos-suggestie na `leg uit <functie>`** (vervolgstap op de nieuwe uitlegmodule, `modules/math/math_uitleg.py` — zie de eigen sectie "✅ math_uitleg.py" verderop voor het volledige verslag) — nog niet gestart, ontstaan tijdens het ontwerpgesprek voorafgaand aan het bouwen van die module. Idee: als Kevin net `leg uit X` gebruikte en daarna een bewerking/expressie typt, checkt Nova of die bewerking qua vorm bij functie X past (bv. een expressie met één variabele na `leg uit nulpunt`) en stelt voor: "Wil je dat ik dit oplos met X?". Past de bewerking beter bij een ANDERE functie Y, dan zegt Nova dat expliciet: "Dit is geen X-bewerking maar lijkt op Y — wil je dat ik hem daarmee oplos?" Puur symbolisch: geen vrije-taal-begrip nodig (dus geen LLM), enkel vorm-matching (aantal variabelen, wel/geen graaf-structuur, wel/geen y0/van/tot-argumenten) gecombineerd met het al-bekende `pending_question`-patroon voor de bevestiging. Bewust onderscheiden van een eerder overwogen, bredere versie ("welke formule is het beste bij een vrije, natuurlijke-taal-vraag") — die versie werd verworpen omdat vrije tekst naar functiekeuze vertalen buiten wat trefwoord-matching betrouwbaar aankan valt (zou een LLM vereisen, dus niet passend als kernfunctionaliteit). Nog te bepalen: exacte vorm-matching-regels per functie, en of dit meteen voor alle uitleg-functies geldt of ook eerst als kleine pilot.
24. 🔴 **Typewriter-effect + achtergrondthread: door elkaar gehusselde tekst bij lange antwoorden** — ontdekt tijdens het live testen van de uitlegmodule (`math_uitleg.py`, zie de eigen sectie "✅ math_uitleg.py" verderop, 5 aug 2026). Bij `leg me uit runge kutta` verscheen een deel van Nova's antwoord zichtbaar gehusseld op tekenniveau (bv. "Waarom dit wiskundig klopt:Nova: W adcohotr, 4j ek ebeern..." — twee stukken tekst die letter-voor-letter door elkaar geregen zijn). Dit gebeurde niet bij math_uitleg.py's andere, even lange antwoorden in dezelfde testsessie (newton, dijkstra) — vermoedelijk een race condition tussen het bestaande typewriter-effect (het geleidelijk uittikken van een antwoord, zie het "verse prompt"-mechanisme/`wachten_op_input` hierboven) en een gelijktijdige achtergrondthread-publicatie (`achtergrond_loop()` in `main.py`, bv. session_watcher/emergence_engine/weather) die ergens halverwege het uittikken een eigen bericht print. Math_uitleg.py's teksten (2600-2900 tekens) maken dit zichtbaarder dan voorheen simpelweg omdat het typewriter-effect nu veel langer duurt per antwoord, waardoor de kans op een overlappende achtergrondpublicatie toeneemt — het onderliggende probleem zit dus niet in math_uitleg.py zelf, maar in hoe het typewriter-effect en `on_chat_response()`/proactieve publicaties elkaar kunnen overlappen. Nog te onderzoeken: of het bestaande `wachten_op_input`-vlagmechanisme enkel de PROMPT beschermt (zoals ontworpen) maar niet een al-lopend typewriter-effect zelf onderbreekt/pauzeert, en of een simpele lock/queue rond het printen van elk antwoord dit zou oplossen.
