...
6. 🟢 **Layer 1 — 2 nog openstaande "Advanced queries" uit memory_layer1_roadmap.md** — bij de fase-controle (29 juli 2026) gevonden als overgebleven gaten in een verder volledig afgeronde Layer 1. `find_bridge()` intussen AFGEROND (9 augustus 2026, zie nova_changelog.md) — de twee resterende:
...
6b. 🟢 **`find_bridge()` koppelen aan Nova's echte gesprekslogica** — momenteel enkel bereikbaar via het debug-commando `bridge <w1> <w2>` (9 augustus 2026), niet vanuit een normaal gesprek. Twee losse, niet-overlappende vervolgstappen mogelijk: (a) toevoegen aan `response_engine.generate()`'s `_sterkste_associatie()`-verrijking, naast de al werkende `find_related()`, zodat definitie-antwoorden er ook gebruik van maken; (b) een eigen intent (`detect_bridge_query()`, bv. "wat hebben X en Y gemeen") die `find_bridge()` rechtstreeks aanroept. **Expliciet GEEN duplicaat van het al afgeronde `compare_concepts()` (Idee #6, reasoning_engine_ideeen_roadmap.md)** — `compare_concepts()` vergelijkt FORMELE, geleerde kennis uit `concepts.json` (is_a/part_of/property_of, moet expliciet aangeleerd zijn), `find_bridge()` vergelijkt STATISTISCHE co-occurrence uit ruwe gesprekstekst (Layer 1, ontstaat vanzelf zonder expliciet leren) — complementaire databronnen, geen overlap.
...
24. 🟡 **Automated eval voor het tombstone-mechanisme** — extern advies (gecombineerde review, 6 aug 2026) stelde voor om specifiek te testen of `status == "rejected"` intact blijft na toekomstige code-wijzigingen of nieuwe Wikipedia/auto-extract-zoekacties — dus dat een afgewezen feit nooit stilzwijgend terugkomt. `scripts/test_atomic_save.py` bestaat al als voorbeeld-patroon (los testscript, draait op een tijdelijke kopie van `concepts.json`, nooit het origineel) maar test iets anders (crash-bestendigheid van `save()`). Idee: een vergelijkbaar, apart testscript of een `assert`-gebaseerde test (zie ook punt 28, testsuite-zwakte) die het scenario uit `nova_changelog.md` naspeelt — `hond is_a meubel` → weerleg → contradicties weg → probeer opnieuw dezelfde relatie toe te voegen via `add_relation`/Wikipedia-achtige extractie → bevestig dat de dedup-logica in `add_sense`/`add_relation` de rejected-rij herkent en niet stilzwijgend laat herleven. **Update (8 aug 2026):** test gebouwd (`test_tombstone.py`, oorspronkelijk 6 tests, later uitgebreid) — bevestigde de vermoede zwakte meteen, gedocumenteerd als bug #32 in de bugtabel. **Update 2 (8 aug 2026):** bug #32/#36 volledig opgelost (zie `nova_changelog.md`), inclusief een apart gevonden en gefixte vervolgbug in `wikipedia_teacher.py`. `test_tombstone.py`'s oude, bug-vastleggende test is vervangen door tests die het nieuwe, correcte gedrag vastleggen. Dit punt is daarmee afgerond.
25. 🟢 **Debug-commando `audit <woord>` — audit-log doorzoeken/filteren** — extern advies stelde een aparte inspectie-CLI voor om `concepts.json`/audit-logs te doorzoeken zonder handmatig het JSON-bestand te doorspitten. Bewust NIET als los CLI-script gebouwd — dat zou een tweede toegangsweg naast de chat betekenen, wat ingaat tegen Nova's kernprincipe dat alles via het gesprek zelf loopt (en tegen de "cognitive coupling"-voorzichtigheid van punt 30: twee plekken die dezelfde data tonen). De bestaande inspectiemiddelen (`concept_overview.py` voor "wat weet je over X", `debug_commands.py` voor `geheugen stats`/`contradicties`/enz.) dekken al veel, maar geen ervan laat je specifiek filteren op audit-log-inhoud (bv. "toon alle relaties afgewezen deze maand", "toon alle concepten met source=wikipedia"). Idee: dit als extra debug-commando toevoegen aan de bestaande `debug_commands.py`-dispatch (zelfde patroon als de andere 17 commando's), niet als apart script.
...
`C:\Nova_AI\tests\`. Draaien vanuit de project-root: `pytest tests/ -v`. Stand 8 augustus 2026: **148 tests, allemaal groen** (~6s).

Startpunt was een externe review (5 assertions in 21 print-gebaseerde scripts, plus een `SystemExit` die `pytest tests/` liet crashen bij collectie) — sindsdien in drie rondes opgebouwd, telkens risico-gebaseerd geprioriteerd (kans op stille regressie, niet moduleomvang):
