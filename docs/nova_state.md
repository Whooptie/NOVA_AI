...
**Let op (16 augustus 2026):** bewuste keuze om GEEN nieuwe modules te bouwen die (a) fysieke apparaten/sensoren aansturen of aflezen (smart home, Plex, MIDI, Windows-vensternamen), OF (b) een nieuw ML-model/embedder integreren (downloaden, inpluggen in module_loader.py, paden instellen) zolang de Unraid-server er niet is — beide zouden het integratiewerk twee keer vereisen: eerst nu, dan opnieuw na de verhuizing naar Unraid (andere paden, mogelijk Docker-volumes). Punten hieronder die dit raken zijn gemarkeerd met 🏠. Zuivere taal/geheugen/redeneer-logica die GEEN nieuw model/apparaat vereist (Layer 0-7 uitbreidingen, semantic-regels, intent_router-logica) blijft gewoon oppakbaar, ongeacht waar Nova draait.
...
5. 🏠🟢 **Layer 5 — 3-4 nooit-gebouwde stukken uit memory_layer5_roadmap.md** — bij de fase-controle (29 juli 2026) gevonden als enige overgebleven gaten in een verder volledig afgeronde Layer 5 (alle 5 fases zelf zijn af en zelfs uitgebreider dan gepland). Heroverwogen (15 augustus 2026): bewust NIET als drie losse, gelijkwaardige taken behandelen — zie per stuk hieronder.
   ...
   ├── `screen_focus` — de specifieke actieve applicatie/vensternaam (bv. "vs_code") als apart veld naast het bestaande, afgeleide activiteitslabel. `activity_detector.py` leest de venstertitel al uit voor de labelvertaling, bewaart de rauwe titel zelf enkel nog niet apart. **Bewust nog open gelaten (15 augustus 2026, niet afgesloten):** Kevin schat in dat toekomstige modules (bv. Layer 7-verrijking, "je was lang in VS Code bezig" i.p.v. enkel "je was aan het coderen") dit zouden kunnen gebruiken — in tegenstelling tot pure dode-data-gevallen (`identity_state.json`) is hier al een aannemelijke toekomstige aanroeper. Kleine, puur symbolische toevoeging aan een bestaand bestand, geen nieuwe module nodig.
   └── `get_relevant_topics()` — methode die relevante onderwerpen teruggeeft op basis van de huidige activiteit (bv. `["code", "python", "debugging"]`). Nergens vermeld of gebruikt, maar sluit inhoudelijk aan bij `topic_suggestions.py` (Fase 5) — mogelijk relevant zodra activiteit-gekoppelde topic-suggesties (i.p.v. enkel tijd-gebaseerde) ooit gebouwd worden. **Bewust nog open gelaten (15 augustus 2026, niet afgesloten)**, zelfde reden als `screen_focus`. Puur symbolisch, kleine uitbreiding op `context_manager.py`.
   Geen van deze drie is kritiek — Layer 5's kernfunctie (interruption-logic) werkt volledig zonder ze. `device_mode` wacht op de toekomstvisie-switch; `screen_focus`/`get_relevant_topics()` blijven open staan als kleine, lage-inspanning toevoegingen met een plausibel toekomstig gebruik, geen prioriteit.
