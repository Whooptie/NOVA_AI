...
- **Toekomstige uitbreiding, UITGESTELD (9 juli 2026): Layer 4 laten gelden voor de andere semantic-gerelateerde intents.** Momenteel gebruikt ENKEL `detect_definition()` in `intent_router.py` Layer 4 (`response_engine.generate()`) — alle andere semantic-intents (`intent_synonym`, `intent_antonym`, `intent_used_for`, `intent_causes`, `intent_properties`, `intent_related_to`, `intent_relation_check`) gaan nog altijd rechtstreeks naar hun eigen handler in `chat.py`, zonder Layer 1 (personal touch) of Layer 2 (timing-hint). Dit was nooit een bewuste architectuurkeuze — Layer 4 is gewoon enkel rond de definitie-usecase gebouwd en getest. Bewust NIET uitgebreid naar weer/tijd/math/schaken/help — die modules hebben geen semantic/word_associations/pattern_matcher-achtige databronnen om te combineren, dus Layer 4's kernidee (drie kennislagen samenbrengen) heeft daar inhoudelijk niets te doen. Wel een logische kandidaat voor uitbreiding: de synoniemen/antoniemen/relaties/eigenschappen-intents, die al `semantic.py` gebruiken en dus qua databronnen goed aansluiten bij wat Layer 4 al doet.
...
2. 🟡 **Personality pipeline (deel 2, nog open)** — synoniemen/antoniemen/relaties/eigenschappen-intents in `chat.py` zijn logische volgende kandidaten (gebruiken al `semantic.py`, sluiten qua databron goed aan bij Layer 4). Schaken en help blijven bewust rechtstreeks op `chat_response` (zie `layer4_response`-sectie) — schaken wordt herbekeken zodra Kevin's huidige chess-uitbreidingen klaar zijn.
...
10. 🟡 **emotion_engine.py decay/recovery-mechanisme** — `overstimulation.level` lijkt structureel hoog te blijven staan zonder ooit te dalen (zie identity_state.json-regel hierboven en Layer 4-sectie). Nog niet onderzocht — vermoedelijk hoort dit bij Layer 6.
11. 🔴 **Fundamentele voorwaarde voor proactief gedrag: `main.py`'s blocking `input()`-lus.** Besproken 9 juli 2026: Nova kan momenteel NOOIT spontaan spreken (proactieve suggesties, weerwaarschuwingen, Activity Awareness-suggesties, ...) en kan tijdens een gesprek ook geen eigen initiatief nemen los van een directe reactie, omdat `main.py`'s hoofdlus vastzit in een blocking `input()`-aanroep — zolang Kevin niet typt, gebeurt er letterlijk niks op het scherm, ook al lopen achtergrondtimers (`memory.py`, `pattern_matcher.py`) wel door. Dit is een vrij fundamentele wijziging aan `main.py`'s kernstructuur (bv. via `threading` — een aparte inputthread naast de hoofdthread, of een async-architectuur), niet iets dat je "erbij plakt". Blokkeert momenteel: het al goedgekeurde proactieve-weerwaarschuwing-idee (zie Weather-sectie), Activity Awareness se proactieve suggesties, en de geplande Layer 5 (Context Manager, "mag ik nu onderbreken?"). Nog niet ingepland — eerst apart te ontwerpen wanneer een van deze afhankelijke features aan de beurt is.
...
- Ook nog niet meegenomen: de synoniem/antoniem/relatie/eigenschappen-intents in `chat.py` (zie de "UITGESTELD"-notitie bij Layer 4 hieronder — logische kandidaat voor latere uitbreiding).
