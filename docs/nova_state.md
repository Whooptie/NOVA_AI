...
| calendar.py | ✅ Klaar (nieuw, 16 september 2026) | Dag/datum/maand/jaar/week + algemeen overzicht ("wat is het vandaag"). Zelfde patroon als time.py: gebruikt `zone.now_local()` indien beschikbaar, antwoordt via `layer4_response`. Eigen vaste `DAGNAMEN`/`MAANDNAMEN`-vertaaltabellen i.p.v. `locale.setlocale()`/`strftime("%A"/"%B")` — voorkomt een omgevingsafhankelijke bug (Nederlandse locale niet gegarandeerd geïnstalleerd in de Docker-container). Gerichte vragen (dag/datum/maand/jaar/week) geven elk een kort, specifiek antwoord; enkel de algemene vraag geeft het volledige overzicht in één zin. `detect_calendar()` in `intent_router.py` toegevoegd aan `_build_intent_tabel_deel1()`, vóór `time`. **Bekende, bewust geaccepteerde beperking:** "dag \<naam\>" (bv. "dag Nova") wordt door de losse-woordherkenning opgepikt als kalendervraag i.p.v. door `detect_greeting()` als groet — die laatste deed al enkel een exacte-zin-match (`if text in greetings`) en herkende "dag \<naam\>" dus al nooit, ook vóór deze wijziging. Bewust niet gefixt, Kevin gebruikt deze groetvorm niet. |
