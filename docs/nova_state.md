...
│   ├── context/
│   │   ├── context_manager.py
│   │   ├── activity_detector.py      # sinds 13 sept 2026: RemoteActivityDetector (leest via client_bridge.py)
│   │   ├── focus_detector.py         # sinds 13 sept 2026: RemoteFocusDetector (leest via client_bridge.py)
│   │   └── presence_detector.py      # sinds 13 sept 2026: RemotePresenceDetector (leest via client_bridge.py)
│   ├── network/
│   │   └── client_bridge.py          # WebSocket-server, ontvangt data van nova_client.py (Windows-laptop)
│   ├── preferences/
│   │   ├── kevin_profile.py
...
| module_loader.py | ✅ Klaar | Auto-discovery via pkgutil, laadtijdmeting. **Bugfix (16 augustus 2026, zie nova_changelog.md #42):** `emergence_engine` ontbrak op de generieke dynamische-scan-uitsluitingslijst (naast `topic_suggestions`, die daar al wel op stond) — de scan riep daardoor stilzwijgend `init_module(event_bus, sem)` aan vóór de latere, correcte `layers=`-aanroep, wat een eerste, kapotte `EmergenceEngine`-instantie opleverde (`self.layers` = het `sem`-object i.p.v. een dict). Jarenlang onschadelijk gebleven (die instantie subscribete nergens op), tot een latere wijziging in `emergence_engine.py` een `event_bus.subscribe("*", ...)` in `__init__` toevoegde — toen crashte de kapotte instantie bij het eerste `topic_detected:`-event. `emergence_engine` nu toegevoegd aan de uitsluitingslijst, zelfde patroon als `topic_suggestions`. **Bugfix (20 augustus 2026, derde keer dezelfde valkuil, zie nova_changelog.md — Response Variant Learning):** `variant_feedback_logger` ontbrak eveneens op deze uitsluitingslijst — `init_module(event_bus, sentiment_classifier=None)`'s optionele parameter gaf GEEN `TypeError` bij de generieke `init_module(event_bus, sem)`-aanroep, dus `sem` werd stilzwijgend als `sentiment_classifier` doorgegeven aan een eerste, foute instantie die zich ook al abonneerde op `variant_gekozen`/`raw_user_message` — elke variant-keuze werd hierdoor dubbel gelogd. Live bevestigd (2 identieke regels per keuze in `variant_feedback.jsonl`) en gefixt, zelfde patroon als de twee eerdere gevallen. |
| intent_router.py | ✅ Klaar | Wikipedia, synoniemen, antoniemen, relaties, definitievragen gekoppeld. handle_confirmation() verwijderd als overbodig dood geraamte (30 juli 2026, zie changelog). Nieuw (23 juli 2026):`detect_self_architecture()` — herkent vragen over Nova's werking (geheugen/denken/leren/privacy/architectuur), bewust vóór `detect_identity_question` gecontroleerd, publiceert `intent_self_architecture` (zie self_architecture.py). Nieuw (25 juli 2026): `route()` omgebouwd van 13 losse `if`-blokken naar twee tabellen (`_build_intent_tabel_deel1()`/`_deel2()`, lijsten van `(topic_naam, detect_functie)`) — pure organisatie-refactor, geen enkele `detect_*()`-methode of het gedrag zelf gewijzigd. Nieuw (5 aug 2026): `detect_uitleg()` (math_uitleg.py-koppeling) + `_check_uitleg_vervolg_bewerking()`/`_on_uitleg_oplossen_answered()` (contextuele oplos-suggestie na "leg uit X", hergebruikt `pending_question` met eigen vraag_type `"uitleg_oplossen"`). Nieuw (8 aug 2026, Bug #32/#36): eigen pending-stap **-1F** voor `semantic.py`'s `pending_reactivation` (rejected-sense-heractivatie-vraag), vóór de generieke ja/nee-confirm-check. Bugfix #37 (8 aug 2026): de "andere betekenissen van X"-kernzin-matching zocht per ongeluk op `" van "`/`" voor "` mét voorloop-spatie tegen een al gestripte string — matchte daardoor nooit, viel altijd terug op een mogelijk verouderd `_laatste_definitie_woord`. Nieuw (16 aug 2026, punt 14): `detect_memory_query()` — natuurlijke-taal-toegang tot memory.py's Query API, los van de bestaande kale `detect_memory()`-commando's. Twee paden: met woord → `memory.search()` gefilterd op `event_type == "raw_user_message"`; zonder woord (algemeen) → hergebruikt `word_associations_learner.get_trending()`, zelfde bron als `detect_trending_query()`. Apart aangeroepen in `route()`, eigen `_emit_topic("memory_query", bron="detect")`. Zie nova_changelog.md #41 voor volledige details en testresultaten. |
| memory.py | ✅ Klaar | v2.0 daemon-mode volledig gebouwd (Fase 1-5): portable pad, WAL-SQLite, write-buffering, Query API, achtergrond-onderhoud (archiveren/comprimeren/VACUUM elke 6u). Fase 5 (optimalisatie/polish, 23 juli 2026) ook afgerond: query caching op get_stats() (120 sec + force_refresh), self.recent begrensd op 500 (memory leak-fix), threading.Lock() rond alle self.conn-operaties en gedeelde lijsten (concurrent access), automatische backup elke 6u (laatste 7 bewaard), health_check()-methode. Nieuwe commando's in main.py: 'geheugen stats'/'geheugen stats vers' en 'geheugen gezondheid'. **Sinds 16 augustus 2026 (punt 14, zie nova_changelog.md #41):** `search()`/`query()`/`get_stats()` doorzoeken/tellen nu ook `interactions_old` (gearchiveerd, >90 dagen) via UNION ALL, nieuwe `include_archief`-parameter (default True) om dit optioneel uit te zetten. Zeer oude, gecomprimeerde `.jsonl.gz`-archieven (>1 jaar) blijven bewust buiten bereik van deze functies — apart, later werkpunt. Natuurlijke-taal-toegang tot deze Query API vanuit een gewoon gesprek: zie `detect_memory_query()` in intent_router.py hieronder. |
...
| last_context.py | ✅ Klaar, getest en live bevestigd (22 augustus 2026) | Taal & Redeneerlimieten, idee 1+2 (`taal_en_redeneerlimieten_roadmap.md`) — kortlevend "waar ging het net over?"-geheugen: laatst besproken concept + antwoordtype + laatst aangeboden opties-lijst. Bewust GEEN uitbreiding van `pending_question.py` (andere levenscyclus: doorlopend gespreksgeheugen i.p.v. vraag-verwacht-antwoord). Vangnet-verval na 300 sec, veiligheidsklep (een nieuw, expliciet concept overschrijft altijd, ongeacht de klok), `ververs_timestamp()` verlengt geldigheid zonder de inhoud te wijzigen. Bereikbaar via `event_bus.modules.get("last_context")`. Handmatig geladen in `module_loader.py`, zelfde reden als `pending_question.py`. `intent_router.py` raadpleegt/vult dit via nieuwe stappen 2E/-1G in `route()` (`_verwerk_referentie()`/`verwerk_referentie_antwoord()`) en via `_emit_topic()`'s nieuwe optionele `concept`-argument, incl. de 7 tabel-gedreven detect-functies uit `_build_intent_tabel_deel2()` (vaste regel: altijd de eerst genoemde term). 80 nieuwe tests (`test_last_context.py` + `test_referentie_resolutie.py`), zie `nova_changelog.md` voor het volledige bouwverslag. |
| interruption_tracker.py | ✅ Klaar en getest (22 juli 2026; uitgebreid met Fase 6 tijdsvenster-verfijning, 18 augustus 2026) | Activity-Aware Interaction: houdt per activiteit een confidence-score bij (hoe vaak mocht Nova storen tijdens deze activiteit). API: `record_feedback()`/`get_confidence()`/`get_pattern()`/`has_enough_data()`, elk met een NIEUW optioneel `tijd_sinds_start`-argument (Fase 6). Zonder dit argument blijft alles exact het oude, activiteit-brede gedrag — met dit argument wordt per activiteit ook apart geteld binnen twee tijdsvensters (`"vroeg"` = eerste `VENSTER_GRENS_MINUTEN` (20) minuten sinds activiteit-start, `"laat"` = daarna), zodat Nova kan leren dat storen bv. vroeg in "coderen" meestal niet oké is maar later wel. `has_enough_data()` checkt de drempel (`MIN_OBSERVATIES`, 5) per venster apart — een activiteit kan in totaal genoeg observaties hebben terwijl één venster daarbinnen nog te dun is. Backward-compatible: oude, reeds opgeslagen data zonder `"vensters"`-sleutel wordt bij de eerste nieuwe `record_feedback()`-aanroep alsnog aangemaakt, geen crash. `response_engine.beslis_interruption_gedrag()` en `session_watcher.py`'s aanroep ervan geven dit tijdsvenster nu door (zie changelog voor volledige keten). Getest met 11 nieuwe pytest-tests (`tests/test_interruption_tracker_fase6.py`) — backward-compat, venstergrens exact op 20 min, per-venster `has_enough_data()`, persistentie. Live bevestigd (18 augustus 2026): `interruption test coderen ja 5 10` en `interruption gedrag coderen 30` gedragen zich zoals verwacht. Slaat direct op bij elke `record_feedback()`-aanroep (geen write-buffer, feedback komt van nature weinig voor) in `data/interruption_patterns.json`. Handmatig geladen in `module_loader.py`, zelfde reden als pending_question.py. Zie sectie "Activity-Aware Interaction" verderop. |
...
| calendar.py | ✅ Klaar (nieuw, 16 september 2026; uitgebreid met datumrekenen, 17 september 2026) | Dag/datum/maand/jaar/week + algemeen overzicht ("wat is het vandaag"). Zelfde patroon als time.py: gebruikt `zone.now_local()` indien beschikbaar, antwoordt via `layer4_response`. Eigen vaste `DAGNAMEN`/`MAANDNAMEN`-vertaaltabellen i.p.v. `locale.setlocale()`/`strftime("%A"/"%B")` — voorkomt een omgevingsafhankelijke bug (Nederlandse locale niet gegarandeerd geïnstalleerd in de Docker-container). Gerichte vragen (dag/datum/maand/jaar/week) geven elk een kort, specifiek antwoord; enkel de algemene vraag geeft het volledige overzicht in één zin. `detect_calendar()` in `intent_router.py` toegevoegd aan `_build_intent_tabel_deel1()`, vóór `time`. **Bekende, bewust geaccepteerde beperking:** "dag \<naam\>" (bv. "dag Nova") wordt door de losse-woordherkenning opgepikt als kalendervraag i.p.v. door `detect_greeting()` als groet — die laatste deed al enkel een exacte-zin-match (`if text in greetings`) en herkende "dag \<naam\>" dus al nooit, ook vóór deze wijziging. Bewust niet gefixt, Kevin gebruikt deze groetvorm niet. **Uitbreiding datumrekenen (17 september 2026, date_calendar_roadmap.md Onderdeel 1):** `dagen_tot()`/`dag_van_de_week()`/`datum_plus()` + eigen `_parse_datum()`-herkenning voor "morgen"/"overmorgen"/"vandaag", weekdagnamen (eerstvolgende gelegenheid, nooit vandaag zelf — zelfde regel als weather.py), "over N dagen/weken", cijfernotatie (`15/08`, `15-08-2027`) en dag+maandnaam (`15 augustus`, met automatische jaarsprong naar volgend jaar als die datum dit jaar al voorbij is; "vandaag zelf" telt niet als voorbij). Bewust GEEN feestdagnamen ("kerst"/"pasen") als datum-invoer — dat hoort bij Onderdeel 2, nog niet gebouwd; `_geen_datum_herkend()` geeft dan een nette tegenvraag, geen gok. Bewust GEEN hergebruik van `weather.py`'s bestaande weekdag-/dag-offset-logica (op uitdrukkelijk verzoek, geen wijziging aan weather.py) — hierdoor bestaan de 7 weekdagnamen nu op meerdere, ongekoppelde plekken in Nova (`weather.py`'s `WEEKDAGEN`-dict + zijn `weekdag_namen`-lijst in `dag_label()`, en hier `DAGNAMEN`); bekende, bewust geaccepteerde duplicatie. |
...
| math.py | ✅ Volledig klaar (Fase 1-5, 2 aug 2026) | Berekeningen, temperatuurconversie, wiskundige functies, complete Fase 1 t/m 5: basisrekenkunde/eenheden, Algebra/Calculus/Statistiek, Symbolische algebra (SymPy)/Fysica-engine, Getaltheorie/Complexe getallen/Extra eenheden/CS-algoritmes/Precisie-notatie-exacte vormen — zie math_roadmap.md voor het volledige overzicht |
| math_uitleg.py | ✅ Klaar (5 aug 2026) | Uitgebreide, vaste uitlegteksten per math-functie ("leg uit X"/"wat is X"/"hoe werkt X"). 13 canonieke teksten, 26 gedekte functienamen. Sinds 5 aug 2026 ook een vorm-detectie-laag (`herken_vorm()`/`check_vorm_tegen_verwachting()`/`weergavenaam()`) voor de contextuele oplos-suggestie in `intent_router.py` (7 van de 13 canonieke functies). Zie eigen sectie hieronder voor het volledige verslag. |
| chat.py | ✅ Klaar | Automatische Wikipedia fallback bij onbekend woord. Dode code aanwezig. Nieuw (16 aug 2026, punt 14): `on_memory_query()` — afhandeling van `intent_memory_query` (zie intent_router.py's `detect_memory_query()`), publiceert naar `layer4_response` (niet `chat_response`) zodat het antwoord door de tone-pipeline gaat, zelfde patroon als `on_bridge_query()`/`on_trending_query()`. |
| response_pipeline.py | ✅ Klaar | **Alleen greeting + fallback gaan door personality/tone pipeline — rest nog niet.** Nieuw (8 juli 2026): `on_fallback()` roept nu ook `_auto_learn_from_sentence()` aan — filtert zelfstandige naamwoorden uit de fallback-zin (via `detect_pos`) en slaat onbekende woorden automatisch op als `unknown`-concept via `semantic.auto_learn()`. Functiewoorden (bezittelijke voornaamwoorden, vervoegingen van "gebruiken", etc.) worden expliciet uitgesloten via een stopwoordenlijst — die hoort thuis in Layer 1 (word_associations), niet in concepts.json. Nieuw (28 juli 2026): `PersonalityEngine` wordt hier aangemaakt maar stond nergens in `event_bus.modules` — één regel `event_bus.register_module("personality", self.personality)` toegevoegd, nodig voor `conversation_engine.py`'s `probeer_mood_observatie()`. `on_fallback()` probeert nu eerst `conversation_engine.probeer_mood_observatie()`, dan `probeer_activiteit_observatie()`, voor het op de kale sjabloon-fallback terugvalt. **Nieuw (20 augustus 2026):** de kale sjabloon-fallback (`_sjablonen_fallback`, 5 varianten) gebruikt nu `variant_kiezer.kies_variant()` i.p.v. rechtstreeks `random.choice()` — sjabloon_naam `fallback_algemeen`, zie sectie "Response Variant Learning". |
| conversation_engine.py | ✅ Klaar (Fase 1, 28 juli 2026) | Contextuele fallback-conversatie i.p.v. kale "ik snap dat niet"-zin. Geen event-subscriber (zou een tweede, dubbel antwoord naast response_pipeline.py's on_fallback() opleveren) — publieke methodes die door on_fallback() rechtstreeks aangeroepen worden, elk`None` teruggevend als er niets bruikbaars is. `probeer_mood_observatie()` (Layer 6, via `event_bus.modules.get("personality")`): energiek/rustig/overprikkeld o.b.v. current_energy/overstimulation_level-drempels. `probeer_activiteit_observatie()` (Layer 5, via `context_manager`): activiteit + duur. Beide delen één tijdvenster (`HERHALING_DREMPEL_MINUTEN`, 10) i.p.v. een hard "nooit 2x na elkaar"-slot — anders viel Nova tijdens een lange sessie na 1 observatie voorgoed stil. State (`laatste_praatvorm`/`laatste_observatie_tijdstip`) meteen weggeschreven naar `data/conversation_state.json`, blijft dus ook over een `/reboot` heen bestaan. Bewust NIET gebouwd: topic-/kennisdichtheid-terugkoppeling (Layer 7 doet dit al zelf, zie emergence_engine.py) en een "bodemzin" voor meningsvragen (vereist een nog te bouwen patroonherkenner, geen huidige prioriteit). Live getest: activiteit-observatie bevestigd incl. tijdvenster en reboot-persistentie; alle 3 mood-observatie-takken (energiek/rustig/overprikkeld) bevestigd (1 augustus 2026 — overprikkeld-tak vereiste eerst een sync-bugfix, zie `nova_changelog.md` #28). **Nieuw (20 augustus 2026):** alle `random.choice()`-aanroepen (activiteit-opening/-afsluiting, 3× mood-opening + mood-afsluiting) vervangen door `variant_kiezer.kies_variant()` — elk met een eigen sjabloon_naam (de 3 mood-openingen bewust NOOIT onderling vergeleken, want nooit in dezelfde situatie gekozen). Activiteit-opening gebruikt `uitsluiten_indices` om de `{duur}`-variant te vermijden zonder de variant-lijst zelf te verkleinen. Zie sectie "Response Variant Learning". |
...
| wikipedia_teacher.py | ✅ Klaar | Nederlandse Wikipedia API, disambiguatie-afhandeling, is_a relatie-extractie, automatische fallback vanuit chat.py. Definitie-limiet opgetrokken naar 400 tekens, kapt nooit meer af midden in een woord. Automatische voorbeeldzin-extractie uit Wikipedia geprobeerd maar werkt nog niet betrouwbaar — vervangen door handmatig`example`-commando (zie semantic.py). **Bug #32-vervolgfix#32-vervolgfix#32-vervolgfix#32-vervolgfix (8 augustus 2026):** `_teach_word()`'s niet-`voeg_toe`-pad roept nu rechtstreeks `upgrade_unknown_sense()`/`add_sense()` aan met `source="wikipedia"` i.p.v. via `teach_engine.teach()` + een achteraf-correctie te lopen. De "bestaande Wikipedia-sense overschrijven"-tak checkt nu ook eerst op `status == "rejected"` voordat hij overschrijft — dit werkte rechtstreeks op `concepts.json` en omzeilde de rejected-guard in `add_sense()` volledig, live ontdekt door Kevin (`wiki gitaar` na weerlegging kwam stilzwijgend terug). Zie `nova_changelog.md` bug #36. |
...
| contradiction_checker.py | ✅ Klaar (nieuw, 6 augustus 2026) | Punt 2 — de ontbrekende aanroeper voor`find_contradictions()`. Periodieke achtergrondcheck (`CONTRADICTION_CHECK_INTERVAL_MINUTEN = 15` in `main.py`) die over de volledige kennisgraaf loopt, gevonden tegenstrijdigheden verzamelt en samenvat in één `layer4_response`-bericht met concrete `weerleg:`-suggesties. Eigen spam-preventie via `data/contradiction_state.json` (onthoudt welke conflicten al gemeld zijn, per stabiele woord+conflict-sleutel; opgeloste conflicten worden automatisch weer vrijgegeven). Debug-commando `contradicties` (zie debug_commands.py) forceert de check meteen en toont ALLE conflicten inclusief al-gemelde, zonder de state te wijzigen. Live getest: detectie, samenvattend bericht bij 1 en bij meerdere conflicten tegelijk, spam-preventie over meerdere cycli, en het "opgelost → weer vrij voor een nieuw conflict"-pad — allemaal bevestigd, inclusief live in Nova zelf (zie changelog). |
| topic_suggestions.py | ✅ Klaar (nieuw, 13 augustus 2026) | Punt 7 — topic_events_roadmap.md Fase 5, laatste gat in de topic-events-keten. Periodieke achtergrondcheck (`TOPIC_SUGGESTIONS_CHECK_INTERVAL_MINUTEN = 10` in `main.py`) die per gewhitelist topic (`TOPIC_WHITELIST`, voorlopig enkel `{"chess"}`) `pattern_matcher.is_pattern_active("topic_detected:<naam>")` + `context_manager.can_interrupt()` checkt, en bij een match ongevraagd een sjabloonzin publiceert naar `layer4_response` (bv. "Het is 19u — wil je een potje schaken?"). Eigen, kleine `_topic_naam_labels`-tabel (bewust NIET emergence_engine.py's tabel hergebruikt, die bevat ook topics als "weer"/"rekenen" waarvoor dit sjabloon nooit past). Bewust NIET gekoppeld aan `activity_started:coderen` — ander soort event (afgeleid uit venster, geen expliciete uitspraak), zou raar klinken als trigger. Eigen spam-preventie via `data/topic_suggestion_state.json` (per topic het laatst voorgestelde uur/dag). Afwijkende `init_module(event_bus, pattern_matcher, context_manager)`-signature, handmatig geladen in `module_loader.py` (net als `context_manager`/`emergence_engine`) — vereiste ook een expliciete uitsluiting op bestandsnaam in de generieke dynamische scan, zie changelog voor waarom. Debug-commando's `topic suggesties` (niet-destructieve status via nieuwe `status_nu()`) en `topic suggesties forceer` (echte `check_suggesties()`-aanroep). Live getest: correcte opstart, debug-commando's tonen eerlijke status (patroon nog niet actief, te weinig observaties op dit uur — geen bug). Volledige eind-tot-eind-melding nog niet live gezien, vereist meer natuurlijk opgebouwde data (zie changelog). |
| debug_commands.py | ✅ Klaar (nieuw, 24 juli 2026; uitgebreid 26 juli 2026, 6 augustus 2026, 13 augustus 2026) | Bundelt alle tijdelijke debug-/testcommando's die voorheen los in`main.py`'s invoerlus stonden (~320 regels, zie changelog). Afwijkende `init_module(event_bus, loader)`-signature (krijgt de loader zelf mee i.p.v. `sem`), handmatig geladen in `module_loader.py` net als `pending_question`/`interruption_tracker`/`response_engine`. `main.py` checkt enkel nog `debug_module.is_debug_command(user_input)` en publiceert bij match het `debug_command`-event. Documentatie via nieuw topic `help debug` (`modules/help/topics/debug.py`). Ondertussen 19 commando's: de oorspronkelijke 14 (24 juli 2026), plus `preferences debug` (26 juli 2026), `associaties`/`associaties <woord>` (26 juli 2026, Layer 1 — nodig geworden om de ruis-bug uit `chat_response`-events op te sporen, zie changelog), `contradicties` (6 augustus 2026, punt 2 — forceert de contradiction_checker-check meteen, toont alles inclusief al-gemelde conflicten zonder de spam-preventie-state te wijzigen), `bridge <woord1> en/met <woord2>` (9 augustus 2026, Layer 1 — toont find_bridge()'s brugwoorden tussen twee woorden, zie changelog; syntax later aangepast van `<woord1> <woord2>` naar verplicht `en`/`met` ertussen, zie changelog voor de "en als 2e woord"-bugfix), en `topic suggesties`/`topic suggesties forceer` (13 augustus 2026, punt 7 — respectievelijk niet-destructieve status en een echte geforceerde check van topic_suggestions.py), en `emergence topic <naam>`/`emergence topic <naam> forceer` (16 augustus 2026, punt 15 — zelfde niet-destructief/echt-forceren-onderscheid, nu voor `_check_topic_sterkte()`). Live getest (24 juli 2026): boot OK, `help debug`, `emergence`, `geheugen gezondheid` werken correct. |
...
| activity_detector.py | ✅ Klaar (Layer 5, Fase 2) — **sinds 13 sept 2026: `RemoteActivityDetector`** | Op battleserver sinds de Windows-companion-client (zie changelog) een `client_bridge.py`-vervangklasse i.p.v. zelf te meten (headless Linux heeft geen scherm) — zelfde `detect_activity()`-interface, leest laatst-ontvangen WebSocket-data van `nova_client.py` op Kevins laptop, die de ORIGINELE `pygetwindow`-meetlogica ongewijzigd bevat. Zie "Windows-companion-client"-sectie hieronder voor het volledige verhaal. |
| focus_detector.py | ✅ Klaar (Layer 5, Fase 3) — **sinds 13 sept 2026: `RemoteFocusDetector`** | Zelfde verhuizing als activity_detector.py hierboven — `GetLastInputInfo()` (ctypes) draait nu op Kevins laptop (`nova_client.py`), battleserver leest enkel de laatst-ontvangen waarde via `client_bridge.py`. Geen keylogging — enkel timing, geen inhoud. |
| presence_detector.py | ✅ Klaar (Layer 5, Fase 4) — **sinds 13 sept 2026: `RemotePresenceDetector`** | Zelfde verhuizing — MediaPipe/webcam-detectie draait nu op Kevins laptop (`nova_client.py`, eigen `opencv-python`/`mediapipe`-installatie + hergebruikt `.task`-modelbestand), battleserver leest enkel het laatst-ontvangen `faces_detected`/`is_alone`. Eerste bounded ML-tool in Layer 5, nu dus draaiend op de laptop i.p.v. de server. |
| client_bridge.py | ✅ Klaar (nieuw, 13 sept 2026) | `modules/network/`. WebSocket-server (`websockets`-package), luistert op `0.0.0.0:8765`, ontvangt activity/focus/presence-data van `nova_client.py` op Kevins laptop. Bevat ook de drie `Remote*Detector`-klassen (zie activity/focus/presence_detector.py hierboven). Handmatig geladen in `module_loader.py` (net als `context_manager`/`emergence_engine`), uitgesloten van de dynamische scan. Zie changelog "Windows-companion-client, Fase 1" voor het volledige verhaal. |
| variant_kiezer.py | ✅ Klaar (nieuw, 20 augustus 2026) | Response Variant Learning — puur functioneel bestand (geen klasse, geen state), 1 functie `kies_variant()`. Gedeelde, herbruikbare vervanger voor losse `random.choice()`-aanroepen: haalt gewichten op bij `variant_feedback_logger.py` (indien beschikbaar en genoeg data), kiest gewogen of gelijk, publiceert altijd het `variant_gekozen`-event. Ondersteunt `uitsluiten_indices` (bepaalde indices vermijden zonder de variant-lijst zelf te verkleinen, behoudt stabiele index-betekenis). Gebruikt door `response_engine.py`, `response_pipeline.py` en `conversation_engine.py`. Zie sectie "Response Variant Learning" hierboven. |
| variant_feedback_logger.py | ✅ Klaar (nieuw, 20 augustus 2026) | Response Variant Learning, Fase 1+2 — `modules/response_learning/`. Luistert op `variant_gekozen` (logt naar apart `data/variant_feedback.jsonl`) en `raw_user_message` (koppelt Kevin's eerstvolgende reactie via de échte `sentiment_classifier.py`, max 1 open koppeling per sjabloon_naam). `get_gewichten(sjabloon_naam, aantal_varianten)` levert gewichten aan `variant_kiezer.py` zodra `MIN_OBSERVATIES_PER_VARIANT = 8` gehaald is. Handmatig geladen in `module_loader.py` (afwijkende `sentiment_classifier`-parameter i.p.v. `sem`), MOET uitgesloten blijven van de generieke dynamische scan (zie module_loader.py hieronder voor de bijbehorende bugfix). Zie sectie "Response Variant Learning" hierboven. |
...
| identity/blueprint/loader.py | ✅ Klaar | `load_identity_blueprint()` — laadt/valideert `identity.json` tegen `schema.json`. **Bugfix (16 augustus 2026, zie nova_changelog.md #43):** kreeg een module-niveau cache — werd voorheen onafhankelijk 2x aangeroepen (`self_query.py` bij import, `PersonalityEngine.__init__()`), zichtbaar als een dubbele "✅ Identity blueprint..."-print bij elke opstart. Eerste aanroep laadt/valideert/print zoals voorheen, latere aanroepen krijgen dezelfde, al-geladen dict terug — geen wijziging nodig geweest bij de aanroepers zelf. |
...
| emotion_engine.py | ✅ Klaar | Trigger-gebaseerde emotie-updates. Sinds 22 augustus 2026: meerdere trigger-bronnen (niet enkel greeting meer, ook via microlearning.py's signaalherkenning — zie testsuite-hoofdstuk), incl. last_trigger-bugfix. |
...
## 🧪 Werkafspraak: testdekking bij nieuwe/gewijzigde modules

Bij elke nieuwe module, en bij een grondige wijziging aan een bestaande module: kort afwegen of een pytest-testbestand de moeite waard is. Niet verplicht voor alles — afwegen op risico bij STILLE regressie (een fout die niet meteen opvalt: kennis-/geheugenlogica, autonome achtergrondmodules die zelf beslissingen nemen) versus modules waar een fout meteen zichtbaar is (UI-tekst, eenmalige scripts).

Gevestigd patroon, zie `tests/`:

- Isolatie via pytest's `tmp_path`-fixture — NOOIT de echte `data/`-bestanden aanraken.
- Eerst checken OF en WANNEER een module I/O doet binnen `__init__()`, vóórdat een fixture geschreven wordt. Sommige modules hebben een injecteerbaar pad-parameter (`MemoryModule`, `WordAssociationsLearner`, `ConceptStore`) — daar volstaat het pad meegeven bij constructie. Andere modules hebben dat niet (`ContradictionChecker`, `PatternMatcher`) en lezen soms al tijdens `__init__()` van schijf — daar moet het pad ofwel na constructie overschreven worden (als het lezen pas bij een latere methode-aanroep gebeurt), ofwel via `monkeypatch` vóór constructie (als `__init__()` zelf al leest). Zie `test_pattern_matcher.py`'s changelog-entry (8 aug 2026) voor een concreet voorbeeld van wat er misgaat als dit fout wordt aangenomen.
- Test op categorie/gedrag, niet op interne implementatiedetails die kunnen verschuiven zonder dat het gedrag zelf fout wordt (bv. exacte ML-confidence-scores).
...
| 32 | `get_trending()` (Layer 1, `word_associations_learner.py`) geeft soms losse woordfragmenten of commandowoorden terug i.p.v. echte gespreksonderwerpen (bv. "morg" i.p.v. "morgen", "teach" — een commandowoord, geen onderwerp) tussen wél geldige resultaten ("weer") in. Bestaande eigenschap van Layer 1's tokenisatie/stopwoorden-logica — `detect_trending_query()` gebruikt dezelfde brondata al sinds 11 augustus 2026 zonder dat dit eerder opviel. Pas ontdekt tijdens het live testen van `detect_memory_query()`'s algemene pad (punt 14, "waar praten we vaak over", 16 augustus 2026) — geen bug van punt 14 zelf, `on_memory_query()` toont enkel door wat `get_trending()` teruggeeft. | word_associations_learner.py (`get_trending()`, en/of de onderliggende tokenisatie/stopwoordenlijst) | 🟢 Laag | 🔲 Open — nog te onderzoeken: striktere stopwoordenfilter, minimale woordlengte, of expliciet uitsluiten van bekende commandowoorden (teach/example/onthoud/...). |
...
*(Bug #34#34 opgelost — 9 september 2026: `which stockfish` gaf niets terug, maar `find / -iname "*stockfish*"` toonde dat `apt-get install stockfish` het binary in `/usr/games/stockfish` had gezet — het Debian-pakket plaatst spelletjes-binaries daar standaard, niet in `/usr/bin`, en die map zat niet in de container-PATH. Geen codewijziging nodig: `chess_engine.py`'s bestaande `STOCKFISH_PATH`-fallback (`os.getenv("STOCKFISH_PATH", "stockfish")`) was hier precies voor gebouwd. Opgelost door `STOCKFISH_PATH=/usr/games/stockfish` als environment variable toe te voegen aan de `nova-ai`-container in Unraid. Live bevestigd: `[CHESS] Stockfish gestart` verschijnt, en Stockfish speelt correct terug ("pion naar e5" na "pion naar e4"). Let op bij een toekomstige herinstallatie/nieuwe container: deze variabele moet opnieuw ingesteld worden, ze staat niet in Git/code.)*
*(Bug #33#33 opgelost — 9 september 2026: oorzaak was een woordbotsing in `_REFERENTIE_FUNCTIEWOORDEN` — "weer" stond daar als bijwoord ("doe het weer"), maar is óók het zelfstandig-naamwoord-onderwerp van weervragen. Bij "wat is het weer" werden alle vier de woorden dus als "leeg" gezien (wat/is = functiewoorden, het = verwijswoord, weer = ook al functiewoord) en de zin gold onterecht als kale verwijzing. Structurele fix (i.p.v. enkel "weer" uit de lijst halen): nieuwe `_EIGEN_ONDERWERP_WOORDEN`-set + `_heeft_eigen_onderwerp()`-check, vroeg in `_is_kale_verwijzing()`, VOOR de generieke functiewoorden-lus — een zin met zo'n woord is nooit een kale verwijzing, ongeacht de rest van de zin. Uitbreidbaar voor toekomstige, gelijkaardige woordbotsingen. Live bevestigd: "wat wordt het weer morgen" gevolgd door "wat is het weer" geeft nu correct het weer van VANDAAG (geen "→ referentie opgelost"-regel meer in de debug-log). Regressie-getest: "wat is een gitaar" gevolgd door "wat is dat" wordt nog steeds correct herschreven naar "wat is gitaar" — echte kale verwijzingen blijven dus werken. Bijvangst, GEEN onderdeel van deze bug: "wat is dat" na een schaak-commando ("bord") lost niet op — chess_engine.py roept `last_context.set_concept()` blijkbaar nooit aan, dus dit is een ontbrekende koppeling in de schaakmodule, geen referentie-resolutie-bug. Zie "Volgende stappen" voor dit nieuwe, losse aandachtspunt.)*

---

## 🗑️ Verwijderpad, Trust State & Contradiction Checker (referentie)

Zie `nova_changelog.md` voor het volledige traject (probleem, bugfixes, testresultaten). Hieronder enkel de huidige werking.

### Trust state

Elke sense en relatie in `concepts.json` heeft een `status`-veld naast `source`/`confidence`:

- `"confirmed"` — source is `"user"`
- `"unverified"` — elke andere source (`auto`, `auto_extract`, `wikipedia`)
- `"rejected"` — afgewezen (zie verwijderpad)

Onafhankelijk van het bestaande `definition == "unknown"`-sentinel-patroon, dat ongewijzigd blijft.

### Verwijderpad

Twee niveaus, in `SenseEngine`/`RelationEngine` + `SemanticConceptsModule`-facade:

| Methode | Werking |
| ---- | --- |
| `reject_sense(word, sense_id, reason="")` | tombstone — status → rejected |
| `reject_relation(word, sense_id, relation_type, target, reason="")` | tombstone |
| `reject_concept(word, reason="")` | tombstone, alle senses ineens |
| `hard_delete_sense(word, sense_id)` | fysiek weg, guard: moet al rejected zijn |
| `hard_delete_relation(word, sense_id, relation_type, target)` | fysiek weg, guard |
| `hard_delete_concept(word)` | fysiek weg, guard: alle senses moeten rejected zijn |

Reasoning negeert overal `status == "rejected"`: `RelationEngine.get_relations()` (kernfix, waar `is_a()`, `is_a_chained()`, `part_of_chained()`, `causes_chained()`, `find_contradictions()`, `part_of()`, alle `get_synonyms/antonyms/...()` doorheen gaan), plus `get_all_relations()`, `get_best_definition()`, `detect_sense()`. `get_senses()`/`export_concept()` blijven bewust alles tonen (transparantie).

Chat-commando's (`intent_router.py`, `handle_weerleg()`):

- weerleg: `<woord>` `<relatie_type>` `<target>` -> reject_relation (tombstone)
- weerleg betekenis: `<woord>` `<nummer>` -> reject_sense (tombstone)
- weerleg concept: `<woord>` -> reject_concept, ALLE senses ineens
- verwijder definitief: `<woord>` `<relatie_type>` `<target>` -> hard_delete_relation (guard)
- verwijder definitief betekenis: `<woord>` `<nummer>` -> hard_delete_sense (guard)
- verwijder definitief concept: `<woord>` -> hard_delete_concept (guard)

`<nummer>` = positie in `get_senses()` (1-indexed). Gedocumenteerd in `algemeen.py`.

### Contradiction Checker

`modules/knowledge/contradiction_checker.py`. `check_contradictions()` draait periodiek via `main.py`'s `achtergrond_loop()` (`CONTRADICTION_CHECK_INTERVAL_MINUTEN = 15`), loopt over alle concepten, meldt nieuwe conflicten samengevat via `layer4_response` met `weerleg:`-suggesties. Spam-preventie via `data/contradiction_state.json` (sleutel: woord + gesorteerde conflict-lijst); een opgelost conflict wordt automatisch weer vrijgegeven.

`alle_contradicties_nu()` — geeft alle huidige conflicten terug zonder de spam-preventie-state te wijzigen, gebruikt door debug-commando `contradicties`.

### ConceptStore.save() — atomische write

`save()` schrijft niet meer direct naar `concepts.json`, maar naar een tijdelijk bestand (`concepts.json.tmp<pid>`) in dezelfde map, met `flush()` + `fsync()`, en wisselt dat pas atomisch om via `os.replace()`. Bij een fout tijdens het schrijven wordt het tijdelijke bestand opgeruimd en de fout doorgegeven — het oude `concepts.json` blijft in beide gevallen (normale afronding of een fout) intact.

Testscript `scripts/test_atomic_save.py`: draait volledig op een tijdelijke kopie van `concepts.json` (nooit het origineel), simuleert een crash midden in het schrijfproces, en bevestigt dat het bestand nadien nog exact hetzelfde en nog steeds geldige JSON is. Live gedraaid tegen Kevin's echte data (258 concepten): beide tests geslaagd.

### Deels open blijvend

`add_relation()`'s hardgecodeerde `confidence: 1.0`/`source: "user"` blijft vast staan — er is geen `source`-parameter, dus elke aanroeper komt binnen als `"user"`/`confirmed`. De trust-state-regel zelf (`source == "user"` → confirmed, anders → unverified) staat er al klaar voor mocht `add_relation()` ooit een `source`-parameter krijgen (bv. voor een toekomstige `auto_extract`-relatie-aanroeper) — zie de code-commentaar op die plek in `semantic.py`.
...
| Layer 2 | pattern_matcher.p | ✅ KLAAR (alle 5 fases) | memory_layer2_roadmap.md |
| Layer 3 | semantic.py | ✅ KLAAR | semantic_roadmap.m |
| Layer 4 | response_engine.p | ✅ KLAAR (Fase 1-5, 7; Fase 6 uitgesteld) | memory_layer4_roadmap.md |
| Layer 5 | context_manager.py + activity/focus/presence_detector.py (sinds 13 sept 2026: Remote*-varianten via client_bridge.py, echte meting op Windows-laptop) | ✅ KLAAR (alle 5 fases — tijd, activiteit, focus, aanwezigheid, gewogen interruption-logic) | memory_layer5_roadmap.md |
| Layer 6 | personality_engine.py | ✅ KLAAR | identity_ROADMAP.m |
...
**Sinds 16 augustus 2026 (punt 15, zie nova_changelog.md #42):** `reflect()` reageert nu ook GERICHT op `topic_detected:*`-events (naast de bestaande periodieke klok) via nieuwe `_on_elk_event()`/`_check_topic_sterkte()`. Nieuwe derde gate naast confidence/timing: cooldown (`EMERGENCE_TOPIC_COOLDOWN_MINUTEN = 15`, per-topic, GEDEELD tussen het gerichte pad en de periodieke klok — `data/emergence_topic_cooldown_state.json`). Zie ook `module_loader.py` hieronder voor een tijdens deze sessie gevonden en gefixte, langer bestaande dubbele-instantie-bug.
...
- **Bug #10#10#10#10#10#10#10#10 — ✅ OPGELOST (26 juli 2026):** zie `nova_changelog.md` voor het volledige overzicht van de sense-disambiguatie-oplossing.
- **Emotion/overstimulation-observatie:** tijdens Layer 4-testen viel op dat Nova's `emotion_state.json` structureel op een hoge `overstimulation.level` staat, waardoor elk antwoord het "overprikkeld_chaotisch_snel"-sjabloon (😵⚡💥) kreeg, ongeacht onderwerp. Geen Layer 4-bug — hoorde bij Layer 6, niet bij Layer 4. **Opgelost (11 juli 2026), zie bug #4/#4/#4/#4/#4/#4/#4/#4/#11#11#11#11#11#11#11#11.**
- **Toekomstige uitbreiding, DEELS OPGELOST (12 juli 2026, zie werkpunt #2#2#2#2#2#2#2#2 hierboven): Layer 4 laten gelden voor de andere semantic-gerelateerde intents.** Twee aparte dingen om uit elkaar te houden: (1) **de tone-pipeline** (emoji's/stemming via `expression_injector.py`) — dit is nu opgelost, alle semantic-intents publiceren `layer4_response` en klinken dus warm. (2) **De échte Layer 4-combinatie** (`response_engine.generate()`, die semantic + word_associations (Layer 1, "personal touch") + pattern_matcher (Layer 2, timing-hint) samenbrengt) — dit blijft ENKEL gekoppeld aan `detect_definition()`'s hoofdroute. **AFGESLOTEN, bewust NIET uit te breiden (15 augustus 2026):** de andere semantic-intents (`intent_synonym`, `intent_antonym`, `intent_used_for`, `intent_causes`, `intent_properties`, `intent_related_to`, `intent_relation_check`, `intent_part_of_check`, `intent_subtypes_query`) blijven bewust rechtstreeks naar hun eigen handler in `chat.py`/`semantic.py` gaan, zonder Layer 1/Layer 2-verrijking. Reden: deze intents vervullen in de praktijk drie verschillende rollen — (a) verificatie/debug (checken of `semantic.py` correcte data teruggeeft), (b) gewone eindgebruikersvraag, (c) praktisch naslagwerk (bv. woorden opzoeken voor kruiswoordraadsels). Kevin gebruikt ze vandaag vooral voor (a) en (c), en beide zijn juist gebaat bij een kale, snel scanbare, ondubbelzinnige lijst — een ingevoegde associatie/timing-hint zou daar ruis toevoegen in plaats van waarde. Enkel `detect_definition()` is typisch een volwaardige, conversationele uitspraak waar de volledige Layer 4-combinatie zinvol is. Geen codewijziging, blijft zoals het is. Bewust ook nooit uit te breiden naar weer/tijd/math/schaken/help — die modules hebben geen semantic/word_associations/pattern_matcher-achtige databronnen om te combineren.
...
- **Bug #10#10#10#10#10#10#10#10 (sense-disambiguatie, "python" als slang vs. programmeertaal) — ondertussen ✅ OPGELOST (26 juli 2026, zie `nova_changelog.md`).** Deze per-woord-timing-uitbreiding zelf werkt nog steeds per WOORD, niet per SENSE — geen wijziging hier nodig, puur een statusupdate van de verwijzing.
...
#### Layer 5 — Windows-companion-client: activity/focus/presence verhuisd naar Kevins laptop (Fase 1 afgerond, 13 september 2026)

**Belangrijk om te weten bij het lezen van Fase 2-4 hierboven:** die secties beschrijven de OORSPRONKELIJKE bouw, toen Nova nog rechtstreeks op Kevins Windows-PC draaide. Sinds de verhuizing naar battleserver (Bug/mijlpaal #44 in de changelog) is dat niet langer letterlijk waar — een headless Linux-container heeft geen scherm, muis/toetsenbord of webcam. De ONDERLIGGENDE meetlogica, drempels en architectuurprincipes die hierboven beschreven staan, zijn nog altijd correct en ongewijzigd; enkel WAAR die metingen fysiek plaatsvinden is veranderd.

**Wat er concreet veranderd is:** `activity_detector.py`/`focus_detector.py`/`presence_detector.py` op battleserver zijn vervangen door `Remote*`-klassen (zie `client_bridge.py`, `modules/network/`) die exact dezelfde methode-namen/interface aanbieden, maar de laatst-ontvangen data via een WebSocket-verbinding teruggeven i.p.v. zelf te meten. De ECHTE meetlogica (ongewijzigd overgenomen) draait nu in een los bestand, `nova_client.py`, op Kevins Windows-laptop — buiten de Nova-daemon, buiten `module_loader.py`. `context_manager.py` en `module_loader.py`'s `context_layers`-dictionary zijn hierdoor NIET aangepast — voor Layer 5 verandert er niets aan de interface.

**Volledig traject, inclusief drie niet-triviale bugs (race-conditie bij herverbinding, event-loop-blokkade door synchrone Windows-API-calls, presence die nooit ververste door een vervallen ontwerpaanname) en de uiteindelijke oplossingen: zie `nova_changelog.md`, sectie "Windows-companion-client, Fase 1".**

**Status:** Fase 1 (activity + focus + presence, sensor-only, geen commando's terug) volledig afgerond en live getest — LAN + Tailscale, laptop-herstart-bestendig (Windows Taakplanner), bestand tegen langdurige netwerkuitval (60 min wifi-test doorstaan). Volgende fases (nog NIET gebouwd): Deel A (commando's van Nova naar de laptop, bv. apps openen), Deel B (zelfde soort client voor de telefoon), en ooit een chat-/GUI-laag bovenop dezelfde WebSocket-verbinding — zie `client_server_control_roadmap.md`.
...
| --- | --- | --- |
| /reboot commando (full restart, ~5 sec) | ✅ Klaar en volledig getest (Windows + Docker/Unraid, zie Docker-kanttekening onderaan) | reboot_hotreload_roadmap.md |
...
6. **(Docker/Unraid-specifiek, ontdekt na de migratie, opgelost 13 september 2026):** `/reboot` liet in de chat zien alsof Nova netjes herstartte (afscheidsboodschap + `subprocess.Popen`-nieuwstart), maar de `nova-ai`-container zelf stopte op dat moment gewoon volledig — geen nieuw Nova-proces kwam meer online. Oorzaak: dit is geen bug in `reboot_manager.py` zelf (dat gedrag is en blijft correct, ongewijzigd sinds 5 juli 2026) — het is een ontbrekende Docker-instelling. Zonder een expliciete restart policy weet Docker niet dat de container zichzelf hoort te herstarten zodra het hoofdproces stopt; voor Docker was het oude proces afsluiten (stap vóór de nieuwe `subprocess.Popen`-start) voldoende reden om de hele container als "gestopt" te beschouwen. Opgelost door `--restart unless-stopped` toe te voegen aan de "Extra parameters" van de `nova-ai`-containerconfiguratie in Unraid (i.p.v. het losse `docker update --restart`-commando, dat enkel de lopende instantie aanpast en niet overleeft bij een toekomstige Force Update/herinstallatie). Live bevestigd: container-status springt bij `/reboot` kort naar "stopped" en dan vanzelf terug naar "started". **Let op bij een toekomstige herinstallatie/nieuw template:** deze parameter moet net als `STOCKFISH_PATH` opnieuw manueel worden ingesteld, ze staat niet in Git/code.

Getest op 5 juli 2026: partij (schaak, inclusief actieve Stockfish-verbinding) en patronen (Layer 2) correct teruggevonden na herstart, alle modules netjes herladen en afgesloten, geen dataverlies, geen achterblijvende processen of vensters. WAL-checkpoint bij afsluiten bevestigd door Kevin (23 juli 2026): `-wal`/`-shm` verdwijnen/blijven op 0 bytes na `exit`/`/reboot`, zoals verwacht. Docker-restart-policy-fix bevestigd door Kevin (13 september 2026).
...
**Kanttekening (13 augustus 2026, tijdens het bouwen van punt 7/topic_suggestions.py):** `topic_suggestions.py` roept `is_pattern_active()` aan als een zwarte doos (geen extra parameters) en zou dus NIET automatisch profiteren van een fijner tijdsraster, ook niet als dit idee ooit gebouwd wordt — zolang `is_pattern_active()` zelf op het bestaande, grovere `hours`-veld blijft werken (wat het voorstel hierboven expliciet als uitgangspunt neemt: additief, geen vervanging). Bovendien is `topic_suggestions.py`'s eigen spam-preventie-sleutel (`f"{vandaag}:{huidig_uur}"` in `check_suggesties()`) HARDGECODEERD op uur-niveau, los van wat Layer 2 zelf doet — een tweede, andere suggestie binnen hetzelfde uur zou dus altijd geblokkeerd blijven door die sleutel, ongeacht hoe fijn Layer 2 zelf ooit gaat tellen. **Zou dit ooit gecombineerd worden** (fijnere, "om het half uur"-suggesties): dan is dat een apart, klein vervolgstukje in `topic_suggestions.py` zelf (spam-preventie-sleutel verfijnen naar een half-uur-blok, en `is_pattern_active()` de nieuwe granulariteit laten meegeven) — niet iets wat vanzelf meegroeit zodra dit Layer 2-idee gebouwd wordt.
...
- `math` — ✅ AFGEROND (5 augustus 2026, zie nova_changelog.md). Kale koppeling (publiceert `{"expr": text}`, zelfde patroon als weather/time), geen tussenvraag nodig — `math.py`'s bestaande `try/except` in `on_math()` vangt elke ongeldige expressie al netjes af. BELANGRIJKE, bevestigde beperking: dit dekt ENKEL zinnen die de classifier op taalgebruik herkent maar die al symbolische syntax bevatten — natuurlijke taal zoals "wat is drie plus vijf" wordt niet vertaald naar `3+5` en faalt dus gewoon netjes bij math.py. Zie "Volgende stappen" punt (NLP-vertaler) voor het aparte, nog te bouwen vervolgpunt.
- `preference` — ✅ AFGEROND (4 augustus 2026, zie nova_changelog.md). Toegevoegd aan het register, maar niet als directe actie zoals de andere 5: `_actie_preference_classifier()` stelt een tussenvraag ("Over welk woord gaat dat?"), omdat de bestaande regex (`_ontleed_voorkeur_zin()`) deze tekst al geprobeerd en gemist had — nog eens dezelfde regex aanroepen kon nooit iets anders opleveren. Zodra Kevin het woord geeft, classificeert `sentiment_classifier.py` de volledige originele zin zelfstandig (werkt zonder vooraf-regex-resultaat) en wordt de voorkeur opgeslagen + bevestigd.

---

## ✅ math_uitleg.py — uitgebreide uitleg per math-functie (5 augustus 2026)

**Aanleiding:** Kevin, 2 augustus 2026 — "ik snap nog steeds maar 10% van al die formules". Het bestaande `modules/help/topics/math.py` (help-overzicht) geeft enkel een korte definitie + voorbeeld-input per functie, geen diepgang.

**Wat er is:** nieuwe module `modules/math/math_uitleg.py`, nieuw intent `detect_uitleg()` in `intent_router.py`. Herkent meerdere natuurlijke vraagvormen ("leg uit X", "leg me uit X", "kan je uitleggen X", "wat is X", "hoe werkt X") via een aliassen-tabel zodat verschillende manieren om naar dezelfde functie te vragen (bv. "nulpunt"/"newton"/"newton-raphson") allemaal bij dezelfde tekst uitkomen. Elke uitleg is een VASTE, vooraf geschreven tekst (geen generatie) in Nova's eigen "ik"-spreekstijl: de formule stap voor stap opgebouwd, WAAROM ze klopt, een dagelijkse-taal-vergelijking, en een doorgerekend voorbeeld.

**Eindstand: 13 canonieke uitlegteksten, 26 gedekte functienamen** — gebouwd in 3 rondes (pilot + 2 uitbreidingen):

- Pilot: `newton`/`nulpunt` (Newton-Raphson), `dv_rk4` (Runge-Kutta 4), `dijkstra` (kortste pad)
- Ronde 2: `regressie`/`correlatie`, `binomiaal`/`normaal`, `levenshtein`, `bfs`/`dfs`, `solve_sym`/`factor_sym`, `projectiel`/`val_met_weerstand`, `afgeleide`/`integraal`
- Ronde 3: `wortel`, `det`/`inverse`, `kans_dobbelsteen`/`kans_kaart`

Nauw verwante functies (bv. regressie+correlatie, bfs+dfs) bewust gecombineerd tot één tekst i.p.v. apart, zodat het verschil tussen de twee er expliciet in staat. Bewust NIET gebouwd (te eenvoudig, weinig meerwaarde boven de bestaande korte help-tekst): `ggd`/`kgv`, `modulo`, `faculteit`, `is_priem`, `naar_binair`/`naar_hex`, basisrekenkunde, eenhedenconversie.

**Architectuur:** module leeft in `modules/math/` (niet `modules/help/topics/` — inhoudelijke domeinkennis, geen Nova-zelfbeschrijving), geregistreerd via een minimale `init_module(event_bus, sem=None)`, opgepikt door `module_loader.py`'s normale dynamische scan. `detect_uitleg()` haalt de module op via `event_bus.modules.get("math_uitleg")`, zelfde patroon als `wikipedia_teacher`/`concept_overview`.

**Layer 2-koppeling:** `detect_help()` en `detect_uitleg()` verhuisd naar hun eigen expliciete stap in `route()` (net als de definition-check), en bewaren nu hun specifieke onderwerp zodat Layer 2 een dynamische topic-naam telt: `help_math`, `help_schaak`, `uitleg_newton`, `uitleg_regressie_correlatie`, enz. Werkt generiek voor elke nieuwe canonieke naam — geen `intent_router.py`-wijziging nodig bij verdere uitbreiding.

**Volledig verslag inclusief live-testresultaten:** zie `nova_changelog.md`, sectie "Nieuwe module: math_uitleg.py".

**Vervolgstap, GEBOUWD EN LIVE GETEST (5 augustus 2026):** contextuele oplos-suggestie na `leg uit X` — voor 7 van de 13 canonieke functies (`newton`/`nulpunt`, `dv_rk4`, `bfs`/`dfs`, `regressie`/`correlatie`, `levenshtein`, `det`/`inverse`, `afgeleide`/`integraal`) checkt Nova nu of een bewerking die net na een uitleg komt, qua vorm past — en stelt voor om hem meteen op te lossen. Past ze beter bij een andere functie, zegt Nova dat expliciet. Nieuwe vorm-detectie-laag in `math_uitleg.py` (`herken_vorm()`), hergebruikt het bestaande `pending_question`-mechanisme voor de bevestiging. Bewust NIET gedekt: `dijkstra` (deelt vorm met bfs/dfs), `solve_sym`/`factor_sym` (een "="-teken alleen is geen betrouwbaar signaal), en de functies die simpelweg "2-3 losse getallen" delen met tientallen andere math.py-functies. Volledig verslag: zie `nova_changelog.md`.

**Nog open, apart genoteerd:**

- Bijvangst-bug (typewriter/threading race condition bij lange antwoorden) — zie bugtabel en "Volgende stappen" hieronder.
...
- ~~Bug #10-koppeling#10-koppeling#10-koppeling#10-koppeling#10-koppeling#10-koppeling#10-koppeling#10-koppeling (disambiguatie voor meerduidige woorden)~~ — ✅ VOLLEDIG AFGEROND (26 juli 2026, zie `nova_changelog.md`): `kevin_profile.py` onthoudt nu welke sense Kevin meestal bedoelt bij een meerduidig woord, via het nieuwe commando `onthoud sense <woord>`.
...
**Historische kanttekening (16 augustus 2026, VERVALLEN sinds 5 september 2026):** hieronder stonden punten gemarkeerd met 🏠 omdat destijds bewust gekozen werd om GEEN nieuwe modules te bouwen die (a) fysieke apparaten/sensoren aansturen of aflezen, OF (b) een nieuw ML-model/embedder integreren, zolang de Unraid-server er niet was — dat zou het integratiewerk twee keer hebben vereist. **Nova draait sinds 5 september 2026 op battleserver (Unraid/Docker, zie changelog Bug/mijlpaal #44)** — deze beperking geldt dus niet langer, en de 🏠-markering hieronder is nog enkel historisch te lezen (waarom een punt destijds uitgesteld werd), geen actuele blokkade meer. Punten die specifiek Windows-gebonden sensoren nodig hebben (bv. vensternamen) lopen sinds de Windows-companion-client (13 sept 2026, zie changelog) via `nova_client.py` op Kevins laptop, niet meer via battleserver zelf.

1. 🏠🟢 **Contextuele suggesties tussen activiteiten** (Activity-Aware Interaction, Deel 4) — nog niet gestart. Puur co-occurrence-tellen zoals Activity Awareness Deel C (bv. Plex → lichten dimmen), maar vereist voor "alledaagse" acties (zoals lichten dimmen via schakelaar) een aparte sensor/integratie-laag (bv. Home Assistant/Hue) om dat moment uberhaupt als Nova-event zichtbaar te maken. **🏠 blijft hier terecht** (los van de Unraid-verhuizing zelf) — de ontbrekende sensor/integratie is Home Assistant/Hue, niet de servermachine. Volledig uitgewerkt in: **interruption_learning_roadmap.md, Deel 4**.
2. 🟢 **Fijner tijdsraster in Layer 2** — concept, nog niet ingepland (zie eigen sectie "💡 Idee (nog niet ingepland): fijner tijdsraster in Layer 2" verderop voor het volledige voorstel en de reden om nu nog niet te bouwen). **Let op (13 augustus 2026):** `topic_suggestions.py` (punt 7) zou hier NIET automatisch van meeprofiteren — zie de kanttekening in die sectie zelf voor waarom, en wat er apart nog zou moeten gebeuren om dat wél te koppelen.
3. 🏠🟢 "Mag tegenspreken, moet feitelijk kloppen" — omzetten van afspraak naar werkende code. (raakt intent_router.py/response_engine.py/semantic.py) **BEWUST UITGESTELD tot na Semantic Fase 8+ (punt 4).** Bij uitwerking (15 augustus 2026) bleek dit drie deelgevallen te hebben, niet één: (a) Kevin beweert iets dat een bestaande `confirmed`-relatie tegenspreekt → Nova mag tegenspreken, onderbouwd vanuit `concepts.json`; (b) Kevin beweert iets waar Nova nog niets over weet → mag NIET zomaar als waar aangenomen/opgeslagen worden (huidige auto-extract-pad zou dit nu wél stilzwijgend doen, confidence 0.9) — vereist eerst een check (bv. Wikipedia-lookup via het bestaande `wikipedia_teacher.py`-pad) vóór opslag; (c) een bewering die niet objectief/symbolisch checkbaar is (subjectief, geen encyclopedisch feit) → Nova kan hooguit aangeven het niet te kunnen verifiëren, nooit blind aannemen of blind tegenspreken. Bewust gekoppeld aan Fase 8+ i.p.v. nu al bouwen: de geplande embedder/ML-uitbreidingen uit Fase 8+ kunnen deelgeval (c) beter afhandelen (gelijkenis/context-afweging bij niet-triviale beweringen) dan wat er vandaag staat — nu al bouwen zou dat stuk later moeten herbouwen (zelfde principe als de embedder-vuistregel: geen tussenstap bouwen als de definitieve aanpak al gepland staat). Let op: een embedder levert ook dan geen hard waarheidsoordeel — dat blijft beperkt tot symbolisch checkbare feiten; de embedder maakt enkel het onderscheid tussen "checkbaar" en "niet-checkbaar" preciezer.
4. 🏠🟡 Semantic Fase 8+ (semantic_extension_roadmap.md blijft leidend) — start met Fase 8 (Causal Reasoning), pure symbolisch, geen ML nodig. `reasoning_engine_ideeen_roadmap.md`'s 6 losse ideeën zijn ondertussen ALLE 6 afgerond (8 augustus 2026, zie nova_changelog.md) — dit punt gaat dus nu enkel nog over de eigenlijke Fase 8+ uit semantic_extension_roadmap.md.
   4b. 🏠🟢 **Idee #4 generiek maken: multi-hop query-combinatiemachine** — `get_all_parts_with_property()` (8 augustus 2026) dekt bewust enkel het specifieke patroon "onderdelen × eigenschap". Vervolgstap, nog niet gepland: een generieke combinatielaag die willekeurige query-paren kan schakelen (bv. subtypes × related_to, "welke soorten dier zijn gerelateerd aan water"), i.p.v. voor elke nieuwe combinatie een aparte, specifieke methode + intent te bouwen. Vereist nadenken over hoe zo'n generieke vraag natuurlijke-taal-herkenbaar te maken is (een vaste triggerzin per combinatie werkt niet meer bij een generieke opzet). Puur symbolisch, geen ML nodig — enkel groter combinatiewerk dan idee #4 zelf.
5. 🏠🟢 **Layer 5 — 3-4 nooit-gebouwde stukken uit memory_layer5_roadmap.md** — bij de fase-controle (29 juli 2026) gevonden als enige overgebleven gaten in een verder volledig afgeronde Layer 5 (alle 5 fases zelf zijn af en zelfs uitgebreider dan gepland). Heroverwogen (15 augustus 2026): bewust NIET als drie losse, gelijkwaardige taken behandelen — zie per stuk hieronder. **`device_mode` hieronder is inhoudelijk hetzelfde patroon als de intussen afgeronde Windows-companion-client (13 sept 2026, zie changelog)** — zou dus vermoedelijk ook via `nova_client.py` (Bluetooth/wifi/batterij zijn immers ook Windows-gebonden) moeten lopen, niet via battleserver zelf.
   ├── `device_mode` — Bluetooth/headphones-detectie, WiFi-netwerk, batterijniveau, PRIVATE/publieke modus. Geen enkel spoor in de huidige code. **Hoort inhoudelijk bij de toekomstvisie "PRIVATE/PUBLIC-gedragsswitch + dual voice" (zie toekomstvisie-sectie) — pas zinvol te bouwen samen met die switch, niet als losstaande sensor zonder aanroeper.** Puur symbolisch haalbaar (OS-API's uitlezen, geen ML), vergelijkbaar met `activity_detector.py`/`focus_detector.py`, maar bewust NIET vooruitbouwen zonder de bijbehorende switch-logica.
   ├── ✅ `screen_focus` — AFGEROND (14 september 2026, zie nova_changelog.md). Rauwe venstertitel wordt nu doorgegeven in Layer 5's context (`ctx["screen_focus"]`), live bevestigd via de Windows-companion-client. Koppeling naar Layer 7 (`emergence_engine.py`, nieuw insight-type `scherm_focus`) AFGEROND en live bevestigd (15 september 2026, zie nova_changelog.md).
   └── ✅ `get_relevant_topics()` — AFGEROND (14 september 2026, zie nova_changelog.md). Nieuwe methode op `ContextManager`, vaste lookup-tabel `ACTIVITEIT_NAAR_TOPICS`, live bevestigd. Koppeling naar `topic_suggestions.py` (tweede, activiteit-gebaseerde trigger naast de bestaande tijd-trigger) AFGEROND en live bevestigd (15 september 2026, zie nova_changelog.md).
   `device_mode` blijft als enige van de drie nog volledig open staan, wacht op de toekomstvisie-switch.
6. 🔴 **Intent Classifier — Layer 1 als extra signaal, GETEST EN BEWUST NIET GEBOUWD (18 augustus 2026).** Geïmplementeerd als losstaand testscript (buiten de echte codebase) en 20x gemeten met een stratified 80/20 train/test-split op de echte `training_data.json` (214 voorbeelden, 11 categorieën) + `word_associations.json` (222 woorden, 742 associaties). Resultaat: kaal TF-IDF haalde 76,3% accuracy; TF-IDF + Layer 1 (simpele top-5-ankerwoorden per categorie) haalde 69,4% (−6,9%); een tweede, strengere versie met onderscheidingsscore i.p.v. kale frequentie haalde zelfs 67,2% (−9,1%). Layer 1 was in slechts 2 van de 20 splits beter.
**Kernoorzaak (niet ankerwoord-kwaliteit, maar databron + datasetgrootte):** `word_associations.json` leert van algemene chatgesprekken, `training_data.json` bestaat uit losstaand verzonnen intent-zinnen — beperkte gedeelde woordenschat (maar 52% van de trainingszin-woorden staat sowieso in Layer 1). Bij 214 voorbeelden zorgen de 11 extra kolommen (1 per categorie) er bovendien voor dat 38% van de zinnen enkel nullen krijgt (geen signaal) en van de rest maar ~53% een correct signaal op de eigen categorie — het model verliest meer aan het compenseren van die ruis dan het wint aan het beetje correcte signaal. Een striktere ankerselectie verkleinde de ruis, maar vergrootte het aandeel "geen signaal" en verslechterde het resultaat verder.
**Heropen-voorwaarde (geen vaste datum, wel een concrete drempel):** dit pas opnieuw overwegen zodra `training_data.json` substantieel gegroeid is via **Layer 0/Fase 6** (`_haal_layer0_voorbeelden_op()`, enkel `bron="detect"`-matches) — dat zijn Kevin's eigen, echte zinnen, dezelfde soort taal als Layer 1 leert, dus de woordenschat-mismatch verkleint vanzelf mee. Aanvullend: `unmatched_intents.jsonl` regelmatig doorlopen en bruikbare nieuwe zinnen handmatig als `training_data.json`-voorbeelden toevoegen (zie ook Scenario 4 "Versneller" in de toekomstvisie — een Kevin-getriggerd LLM-verzoek om varianten op onherkende zinnen te genereren, zou dit sneller kunnen laten groeien). Pas bij een merkbaar grotere en meer diverse trainingsset opnieuw testen met hetzelfde script — niet blind opnieuw bouwen zonder te meten.
7. ✅ **`identity_state.json` — dode velden onderzocht en beslist (22 augustus 2026)** — vervolg op de overstimulation_level-sync-bug (zie `nova_changelog.md` #28). Ditmaal VOLLEDIG geverifieerd tegen de echte broncode (`personality_engine.py`, `behavior_modifiers.py`), niet enkel een vermoeden — zelfde controlepatroon als de `identity.json`-correctie hierboven (1 augustus 2026).
   **Beslissing: velden blijven staan in `identity_state.json` (geen opruiming), intentie hier vastgelegd i.p.v. in het JSON-bestand zelf.** Reden: `identity_state.json` blijft zo puur "wat Nova nu is", zonder documentatie-ruis ertussen.
   **Bevestigd dood (geen enkele lezer/schrijver in `personality_engine.py` of elders):**
   - `current_focus`, `emotional_tone`, `emotional_history`, `mood_shift_tendency`, `puberaal_traits_active` — losse velden, geen van alle heeft een aanwijsbare toekomstige aanroeper-kandidaat. **Blijven dood, geen bouwplan.**
   - `interaction_state` (hele object) — `kevin_priority`/`others_priority` lopen vooruit op een "Nova praat met meerdere mensen"-scenario dat nog niet bestaat. **Blijft dood tot die usecase zich aandient.**
   - `energy_dynamics` (hele object) — bevestigd: `apply_energy_modulation()` gebruikt wel degelijk hardcoded `0.5/0.2/0.3`-gewichten in `behavior_modifiers.py`, niet dit object. **Bewust NIET aansluiten:** dit zou een net gefixte, kwetsbare berekening (zie de 1.2-gewichtenbug en de tussentijdse-clamp-bug, beide hierboven in dit bestand) opnieuw openbreken voor beperkte winst. Blijft dood.
   - `safety_state` (hele object) — **bewust NIET als opruim-bijvangst aansluiten.** Dit zijn veiligheidsflags (`romantic_filter_active`, `dependency_filter_active`, ...); als die ooit echt afgedwongen moeten worden, verdient dat een eigen, doordacht traject, geen losse koppeling tijdens een opschoonbeurt.
   **Bevestigd dood, maar met een aannemelijke toekomstige aanroeper (indien Kevin ooit kiest te bouwen):**
   - `context_flags` (hele object: `is_in_deep_conversation`, `is_processing_complexity`, `is_excited`, `is_confused`, `is_focused`) — zou een pure afleiding kunnen zijn van reeds bestaande data (`current_mood`, `overstimulation_level`, `emotion_engine.state`), zodat andere modules een simpele boolean checken i.p.v. zelf drempelwaarden te herhalen. Lage bouwmoeite, geen nieuwe input nodig.
   - `focus_dynamics` (hele object: `hyperfocus_active`, `hyperfocus_strength`, `task_switch_speed`) — zou kunnen aansluiten op de bestaande Layer 5 activity/focus-detection (`activity_started:*` events), als spiegeling naar `identity_state.json` toe, vergelijkbaar met hoe `overstimulation_level` al gespiegeld wordt vanuit `emotion_engine.py`.
   - Geen van beide is nu gebouwd — Kevin twijfelt nog over concrete usecases. **Bewust NIET gebouwd tijdens dit opruimpunt; enkel de intentie hier vastgelegd voor een eventuele latere sessie.**
   **Niet in de oorspronkelijke roadmap-lijst, maar tijdens dit onderzoek wel als (deels) mogelijk dood aangetroffen:** `behavior_modulation_state.mirroring_intensity` en `behavior_modulation_state.adapt_energy_to_context` (enkel `chaotic_variability_state` daarbinnen wordt bevestigd gebruikt door `personality_engine.py`). **Nog niet onderzocht — buiten scope van dit punt, apart te bekijken indien relevant.**
   **Les bevestigd (zelfde als bij de `identity.json`-correctie):** bij twijfel of een veld dood is, altijd de echte broncode van de enige lezer/schrijver controleren (hier: `personality_engine.py` + `behavior_modifiers.py`) vóór een conclusie of beslissing vast te leggen — niet enkel op basis van het databestand zelf redeneren.
8. 🟢 **Taal & Redeneerlimieten — idee 3+4 nog open** (`taal_en_redeneerlimieten_roadmap.md`) — ontstaan uit een extern AI-advies over hoe ver Nova's taal/redeneren op te rekken zijn zonder LLM-kern. Losse ideeënbak zonder eigen bouwvolgorde (zelfde opzet als `reasoning_engine_ideeen_roadmap.md`). **Idee 1 (contextuele referentie-resolutie) en idee 2 (ellipsis/korte antwoorden) zijn AFGEROND (22 augustus 2026, zie `nova_changelog.md` en de nieuwe `last_context.py`-regel in de moduletabel hierboven).** Nog open:
    ...
    Idee 3 kan volledig los gebouwd worden; idee 4 vereist de meeste zorgvuldigheid. Zie het document voor volledige uitwerking.
9. 🟢 Bi-temporeel maken van persoonlijke data (kevin_profile.py + toekomstige personal_ dates_roadmap.md-data) — NIET voor concepts.json (wereldkennis verandert nauwelijks, extra veld zou daar grotendeels ongebruikt blijven), WEL voor alles wat specifiek over Kevin zelf gaat en dus kan veranderen (voorkeuren, later ook persoonlijke datums/relaties). Idee: valid_from/ valid_until naast het bestaande created_at, zodat Nova onderscheid kan maken tussen "dit was altijd al zo" en "dit is recent veranderd" (bv. "je hield vroeger van thee, sinds juli ben je overgestapt op koffie" i.p.v. enkel de oude waarde overschrijven/ verwijderen). Hangt natuurlijk samen met punt 5 (verwijderpad) — remove_preference() zou van hard verwijderen naar markeren-als-niet-langer-geldig kunnen gaan, wat tombstone + bi-temporeel in één keer voor deze specifieke bestanden oplost. Zou ook Layer 7 (emergence_engine.py) een preciezer, tijdsbewuster verhaal over Kevin kunnen geven dan enkel created_at nu biedt.
10. 🟢 **Natuurlijke-taal-naar-SymPy-vertaler voor math** — ontstaan uit de nieuwe `math`-classifier-koppeling (5 augustus 2026, zie `nova_changelog.md`). Die kale koppeling (`{"expr": text}`, zelfde patroon als weather/time) dekt enkel zinnen die de classifier op taalgebruik als `math` herkent maar die al symbolische syntax bevatten. Concreet bevestigd met een test dat dit NIET voldoende is voor natuurlijke taal: `sp.sympify("wat is drie plus vijf")` faalt met een `SyntaxError` — SymPy begrijpt geen woorden zoals "drie"/"plus", enkel echte syntax (`3+5`). Zou een nieuw stuk werk vereisen: getallen-in-woorden herkennen ("drie" → 3), operatiewoorden naar symbolen vertalen ("plus"/"min"/"maal"/"gedeeld door" → +/-/*//), en zinsstructuren als "zoek de waarde van X, vergelijking ..." omzetten naar de juiste `solve_sym(...)`-aanroep. Nog geen ontwerp, enkel als idee vastgelegd — Kevin's woorden: "eerst optie 1 en maak dan een nieuw puntje voor de volgende stappen lijst voor die nlp".
11. 🟢 **Proactieve datum-terugkoppeling ("vorig jaar op deze datum...")** — nieuw idee, nog niet uitgewerkt. Ander soort feature dan de reactieve `detect_memory_query()` (afgesloten, zie nova_changelog.md #41): niet reactief (Kevin vraagt), maar PROACTIEF — Nova zoekt ongevraagd in interactions.db naar events rond dezelfde kalenderdatum in eerdere jaren, en meldt een sterke overeenkomst ongevraagd (bv. "vorig jaar op deze datum hadden we het ook over X"). Kan sinds 16 augustus 2026 gewoon `memory.search()`/`query()` gebruiken, die doorzoeken sindsdien ook `interactions_old` (>90 dagen) — dat was voorheen een openstaand gat, nu niet meer relevant voor dit idee. Zelfde familie als #9 (topic events) en #21 (gespreks-contextuele proactiviteit) — "iets dat al bekend is, ongevraagd terugkoppelen op het juiste moment", maar dan datum-getriggerd i.p.v. onderwerp-getriggerd. Zou vermoedelijk als een extra periodieke check in achtergrond_loop() passen (zelfde patroon als emergence_engine.reflect()), met een eigen can_interrupt()-check via Layer 5, en hangt af van personal_dates_roadmap.md/date_calendar_roadmap.md voor het datum-vergelijkingsstuk. Nog te ontwerpen: welke confidence-drempel "sterk genoeg" is om dit te melden.
12. 🟢 **Intent router — toekomstige opsplitsing (puur preventief, geen actueel probleem)** — externe review signaleerde dat `intent_router.py` (~3300 regels) langzaam een God-object wordt: intent-detectie, fallback, Wikipedia, voorkeuren, topics, commando's, semantic routing, classifiers en correcties zitten allemaal in één bestand. Nog geen enkel concreet probleem vandaag — expliciet genoteerd als "pas aanpakken als de pijn echt begint", niet als iets om nu te doen. Mogelijke toekomstige opsplitsing als richtlijn alvast vastgelegd: `intent/classifier.py`, `command_router.py`, `knowledge_router.py`, `conversation_router.py`, `preference_router.py`, `intent_registry.py`.
13. 🟢 **Cognitive coupling — architectuurfilosofie voor later, geen taak** — zelfde externe review noemt "cognitive coupling" (steeds meer modules die intern weten hoe andere modules werken, bv. response_engine → loaded_modules, context_manager/emergence → meerdere specifieke layers) als Nova's belangrijkste architecturale risico op langere termijn — niet performance, niet Python, niet SQLite. Nog volledig beheersbaar bij de huidige ~100 modules. Idee voor de verre toekomst (geen ontwerp, enkel vastgelegd zodat het niet vergeten wordt): een centrale "cognitive API"/capability registry, zodat modules met elkaar praten via een vast contract in plaats van elkaars interne structuur rechtstreeks te kennen — relevant zodra Nova richting honderden modules groeit, niet nu.
14. 🟢 **Debug-commando `audit <woord>` — audit-log doorzoeken/filteren** — extern advies stelde een aparte inspectie-CLI voor om `concepts.json`/audit-logs te doorzoeken zonder handmatig het JSON-bestand te doorspitten. Bewust NIET als los CLI-script gebouwd — dat zou een tweede toegangsweg naast de chat betekenen, wat ingaat tegen Nova's kernprincipe dat alles via het gesprek zelf loopt (en tegen de "cognitive coupling"-voorzichtigheid van punt 30: twee plekken die dezelfde data tonen). De bestaande inspectiemiddelen (`concept_overview.py` voor "wat weet je over X", `debug_commands.py` voor `geheugen stats`/`contradicties`/enz.) dekken al veel, maar geen ervan laat je specifiek filteren op audit-log-inhoud (bv. "toon alle relaties afgewezen deze maand", "toon alle concepten met source=wikipedia"). Idee: dit als extra debug-commando toevoegen aan de bestaande `debug_commands.py`-dispatch (zelfde patroon als de andere 17 commando's), niet als apart script.
15. 🟢 **Layer 7 whitelist-onderhoud — proactieve "nieuwe naam gedetecteerd"-melding** (ontstaan uit de whitelist/vertaling-sessie van 30 juli 2026, zie changelog #26). Probleem: Kevin vergeet soms om een nieuwe `activity_started:<naam>` toe te voegen aan `INSIGHT_WAARDIGE_ACTIVITEITEN`, of een nieuwe `topic_detected:<naam>` te vertalen in `_topic_naam_labels` — dit blijft nu onopgemerkt totdat iemand toevallig `patterns_layer2.json` handmatig doorspit (zoals bij de ontdekking van `unknown_gedetecteerd`, total=67). Gekozen aanpak: Nova meldt dit zelf proactief, met een eigen, kleinere observatie-drempel (los van `MIN_OBSERVATIES_VOOR_ANOMALIE`) om te vermijden dat een eenmalig toevallig venster/topic al een melding triggert — voorstel `MIN_OBSERVATIES_VOOR_NIEUWE_NAAM_MELDING`, exacte waarde nog te bepalen (richtwaarde 3-5). Geldt voor BEIDE: nieuwe `activity_started:`-namen (nog niet in de whitelist) én nieuwe `topic_detected:`-namen (nog geen vertaling in `_topic_naam_labels`). Vereist een klein anti-spam-geheugen (naar het patroon van `contradiction_state.json`) zodat eenmaal gemelde namen niet herhaaldelijk opnieuw gemeld worden, tot Kevin ze expliciet toevoegt/vertaalt. Bewust GEEN 5e/6e insight-type in `emergence_engine.py`'s `reflect()` — dat systeem is voor inhoudelijke observaties over Kevin, dit is een systeemonderhoudsmelding; hoort eerder thuis als apart event vanuit `pattern_matcher.py` zelf (waar de telling al gebeurt). Open ontwerpvragen bij het bouwen: exacte drempelwaarde, exacte sjabloonformulering, en of de melding via `layer4_response` (Nova's stem/toon) of `chat_response` (technisch/direct) moet lopen.
16. 🟢 **`intent_router.py` — semantic.py raadplegen bij lage classifier-confidence** (ontstaan uit een Claude-gesprek, 13 augustus 2026). Vandaag valt `route()` bij twijfel enkel terug op `intent_classifier.py` (TF-IDF, vaste lijst van 10 categorieën) — er wordt geen gebruikgemaakt van wat semantic.py al weet. Idee: wanneer de classifier een lage/twijfelachtige confidence teruggeeft, een extra check inbouwen die het kernwoord uit de zin opzoekt in `concepts.json` (via `semantic_api.md`) — bestaat het concept al, en zo ja, welke categorie/relatie hangt eraan vast? Dat kan als extra hint dienen om de routing-gok te verbeteren. **Eerlijke beperking:** dit helpt enkel als het woord zelf al gekend is in semantic.py — bij een volledig onbekend woord verandert er niets, dan blijft de bestaande fallback gelden. Puur symbolisch te bouwen (een extra opzoek-stap in bestaande kennis, geen ML/LLM nodig), maar de exacte plek in `route()`'s volgorde vereist zorgvuldigheid — vergelijkbaar met de eerdere routing-bugs bij `detect_definition()` (bug rond "wat is") en stap 9's relatie-detectie (bug rond "welke onderdelen van keuken zijn scherp"). Nog geen ontwerp, enkel als idee vastgelegd.
17. 🟢 **Nieuwe `detect_verveling()`-intent — reactie op "ik weet niet wat te doen"** (ontstaan uit een CLaude-gesprek, 13 augustus 2026, tijdens het bouwen van punt 7/topic events Fase 5). Bewust onderscheiden van punt 7: dat systeem triggert op TIJDSTIP (Layer 2's `is_pattern_active()`), dit zou triggeren op een HERKENDE, vrije uitspraak van Kevin zelf ("ik weet niet wat te doen", "ik verveel me", "ik heb niks te doen", ...). Idee: een nieuwe `detect_*()`-functie in `intent_router.py` met een vaste lijst triggerzinnen (zelfde patroon als `detect_trending_query()`'s 8 exacte zinnen, geen regex/capture-groups nodig), die bij een match een vaste, sjabloon-achtige suggestie teruggeeft — bv. gebaseerd op `activity_started:werken_aan_nova_gedetecteerd`'s bekende gebruikelijke tijdstip (Layer 2), of een simpele vaste lijst opties (schaken, verder werken aan Nova, ...). Puur symbolisch te bouwen: patroonherkenning op vaste zinnen + sjabloonkeuze, geen ML/LLM nodig — geen vrije generatie, elke gesuggereerde optie moet herleidbaar zijn tot iets dat al bekend is (vergelijkbaar met topic_events_roadmap.md's eigen "eerlijkheid"-principe: Nova verzint nooit zelf een verband dat nergens geteld is). Nog geen ontwerp, enkel als idee vastgelegd — bewust NIET meegenomen in punt 7's bouwsessie, om twee verschillende triggers (tijd vs. herkende zin) niet in dezelfde module te vermengen.
18. 🟢 **`response_engine.py` Fase 7 — personality/tone laten meespelen in `_kies_variant()`** (ontstaan tijdens Response Variant Learning's bouwsessie, 20 augustus 2026). Stond al als docstring-commentaar bovenaan `response_engine.py` ("FASE 7: tone_engine.py/personality_engine.py laten meespelen in welke variant gekozen wordt"), maar nooit als apart, gepland punt in deze lijst — nu expliciet vastgelegd zodat het niet stilzwijgend zoekraakt. **Concrete aanleiding:** Response Variant Learning's `variant_gekozen`-event (Fase 1+2, zie `response_variant_learning_roadmap.md`, inmiddels volledig afgerond) laat het `personality_style`-veld BEWUST leeg (altijd `None`) — `_kies_variant()` heeft vandaag geen toegang tot Nova's actuele personality/tone-staat, enkel tot `sjabloon_naam`/`entity`/`response_style`. Zodra dit punt gebouwd wordt, kan dat veld alsnog gevuld worden (`variant_kiezer.kies_variant()` en `variant_feedback_logger.py` moeten dan hun `entry`-structuur uitbreiden met een `personality_style`-veld, net zoals `response_style` nu al meeloopt) — dat is dan een kleine, aparte uitbreiding op wat vandaag al staat, geen herbouw. Nog geen ontwerp voor dit punt zelf, enkel de koppeling met Response Variant Learning vastgelegd.
19. 🟢 **Windows-companion-client, Deel A/B — commando's terug naar de laptop, en een gsm-variant** (client_server_control_roadmap.md). Fase 1 (sensor-only: activity/focus/presence van laptop naar battleserver) is afgerond, zie changelog. Nog niet gestart: Deel A (Nova stuurt commando's terug, bv. app openen/venster bedienen — vereist een nieuwe module aan de laptop-kant die inkomende WebSocket-berichten omzet naar echte acties, plus een `client_bridge.py`-uitbreiding om iets terug te kunnen sturen, niet enkel te ontvangen), Deel B (zelfde soort client voor de telefoon, ander platform dus andere API's), en een latere, losse chat-/GUI-laag bovenop dezelfde verbinding (naast de huidige stille sensor-only-verbinding).
20. 🟢 **`chess_engine.py` — koppeling met `last_context.py` ontbreekt**(ontdekt als bijvangst tijdens Bug #33's regressietest, 9 september 2026). Een schaak-commando ("bord", "nieuwe partij", een zet) roept nergens `last_context.set_concept()` aan, waardoor een vervolgvraag als "wat is dat" na "bord" niet oplosbaar is (Nova vraagt "Waar heb je het over?" i.p.v. het schaakbord/de partij te herkennen als onderwerp). Referentie-resolutie zelf werkt hier correct — dit is puur een ontbrekende aanroep in de schaakmodule, geen bug in `intent_router.py`/`last_context.py`. Klein, puur symbolisch te bouwen: `chess_engine.py`'s `on_chess_board()`/`on_chess_new()` (of gelijkaardige handlers) zouden na afhandeling `last_context.set_concept("schaakpartij")` (of vergelijkbaar) moeten aanroepen, zelfde patroon als andere modules die dit al wel doen. Nog geen ontwerp, enkel vastgelegd als idee — lage prioriteit, geen gemelde hinder tijdens normaal gebruik.
21. 🟢 **Smart Home + Activity Awareness** (smart_home_roadmap.md + activity_awareness_roadmap.md) — beide volledig uitgewerkt, geen prioriteit toegekend, nog niet in deze lijst opgenomen. Kern: X-Sense rookmelders, Dreame-robotstofzuiger, Samsung Smart TV (elk een eigen, dunne modul, geen Home Assistant nodig — Kevin heeft maar 3 merken); plus een generiek "ik ga <activiteit>"-patroon in intent_router.py, scherm-detectie, co-occurrence tussen activiteiten, en duur-detectie+pauze-suggestie. Sinds de Unraid-migratie is 24/7-beschikbaarheid geen aparte stap meer (volgt vanzelf uit battleserver). Fase 7 van activity_awareness (camera-detectie) is de enige die wél een extern ML-model (vision) vereist, en tevens een apart privacy/consent-ontwerp — de rest is 100% symbolisch.
22. 🟢 **Persoonlijke datums + kalenderrekenkunde + Wikipedia-koppeling** (personal_dates_roadmap.md + date_calendar_roadmap.md). **Onderdeel "welke dag/datum/maand/jaar/week is het" AFGEROND (16 september 2026) + Onderdeel 1 "datumrekenen" AFGEROND (17 september 2026, zie nova_changelog.md) — beide in `calendar.py`.** Nog open, uit date_calendar_roadmap.md: vaste/bewegende feestdagen (Onderdeel 2 — Kerst/Pasen/Nieuwjaar/..., Pasen via het computus-algoritme), schoolvakanties (Onderdeel 3 — handmatige tabel, optioneel Fase 7 automatisch te verversen), en historische datums via Wikipedia's On This Day API (Onderdeel 4) inclusief een "wil je erover lezen?"-keuzeflow (Onderdeel 5) die na bevestiging de juiste Wikipedia-pagina opent via webbrowser.open() — vereist een kleine uitbreiding van pending_question.py met een opties-veld naast het bestaande vraag_type. **Vastgelegde vervolgstap voor Onderdeel 2:** zodra feestdagnamen ("kerst", "pasen", ...) herkend worden, kan `calendar.py`'s `_parse_datum()` daar meteen op aansluiten (`dagen_tot("kerst")` zou dan gewoon moeten werken) — `_parse_datum()` geeft momenteel bewust `None` terug op een feestdagnaam, zie de eigen module-docstring. Voorgesteld als apart bestand (bv. `holidays.py`), niet in `calendar.py` zelf — zie de afweging in nova_changelog.md 16/17 september 2026 (math.py/math_uitleg.py-precedent: aparte taken, apart bestand). Nog volledig open uit personal_dates_roadmap.md: Kevin kan datums laten onthouden ("volgende week maandag examen"), Nova rekent relatieve datums om naar absolute kalenderdatums, en herinnert proactief op de dag zelf/enkele dagen vooraf (zelfde patroon als weather.py's proactieve waarschuwing). **Let op bij Onderdeel 5:** sinds de Unraid-migratie draait Nova's kernproces op battleserver, niet op de laptop — webbrowser.open() zou dus de browser op de SERVER proberen te openen, niet bij Kevin. Dit specifieke stukje moet via nova_client.py lopen (event_bus.publish("laptop:open_url", ...)), net als andere laptop-acties uit punt 19 hierboven, niet rechtstreeks vanuit battleserver.
23. 🟢 **Piano/MIDI-oefenmodule** — nieuw idee (16 september 2026), nog geen roadmap-document, enkel hier vastgelegd zodat het niet verloren gaat. Kern: Kevin wil leren pianospelen (klassiek/soundtracks) op een MIDI-aangesloten piano/keyboard. MIDI-noten zijn al gestructureerde symbolische data (noot, timing, velocity) — geen audio-herkenning nodig, dus geen ML/LLM voor het kernprobleem. Idee: `mido`/`python-rtmidi` leest real-time noten; een referentie-MIDI-bestand (het te oefenen stuk) wordt noot-voor-noot vergeleken met wat Kevin speelt; afwijkingen (verkeerde noot, timing, gemist) worden gelogd naar Layer 0, met Layer 2/7 voor voortgang-patronen ("foutenpercentage in maat 20-24 daalde van 40% naar 12%"). Werkt onafhankelijk van welke lesapp (flowkey/Octaf) Kevin gebruikt — een MIDI-poort kan door meerdere programma's tegelijk gelezen worden (virtuele MIDI-router/loopMIDI), dus Nova heeft geen koppeling met die apps zelf nodig. **Nog te bepalen:** waar dit best draait (battleserver, met de piano fysiek op afstand? of via nova_client.py op een lokale machine waar de piano is aangesloten) — vermoedelijk het laatste, zelfde soort keuze als bij Windows-gebonden sensoren (zie kanttekening bovenaan deze lijst).
24. 🟢 **LLM-codegen-tool als externe, Kevin-getriggerde specialist** (llm_codegen_tool_roadmap.md) — concept, nog niet ingepland. Kern: Claude API als bewust BEGRENSDE, externe tool — nooit als Nova's brein, nooit autonoom aangeroepen tijdens de daemon-loop. Twee toepassingen: (1) fallback/missing-intent tracking blijft 100% symbolisch in Nova zelf (telt hoe vaak en welke woorden bij een fallback terugkeren), (2) Kevin kan expliciet de Claude API laten aanroepen om code te schrijven ("live typen"-effect in VSCode) of om fallback-logs te laten analyseren — altijd Kevin-getriggerd, nooit iets dat Nova zelf beslist te doen. Bevat het kernprincipe "wie beslist wanneer" — de API heeft geen geheugen tussen aanroepen, dus elke call vereist zelf meegestuurde context. Fase 3-6 (de eigenlijke codegen-integratie) wachten op Kevin's bewuste keuze om hiermee te starten.
25. 🟢 **Layer 1 Word Embeddings — aanvulling op PMI** (layer1_word_embeddings_roadmap.md) — concept, open ontwerpvragen. Kern: een extern, voorgetraind (of zelf getraind) taalmodel dat woordvectoren/similarity teruggeeft voor woordparen die NOOIT letterlijk samen voorkwamen in Kevin's gesprekken maar wel betekenisverwant zijn (waar PMI per definitie niets kan vinden, want die telt enkel letterlijke co-occurrence). Draait als bounded specialist (`embedding_specialist.py`, dun bestand zoals `chess_engine.py` rond Stockfish) — woord(en) in, score uit, geen eigen beslissingen. Apart, duidelijk gelabeld `"bron": "embedding"` in word_associations.json, nooit verward met een PMI-associatie. **Nog open:** zelf trainen op Kevin's (kleine) corpus vs. een voorgetraind Nederlands model gebruiken (voorlopig advies: het laatste, gezien corpusgrootte) — vereist nog een korte vergelijking van beschikbare modellen vóór Fase 1 kan starten.
26. 🟢 **Emotional Statement Classifier — apart van microlearning.py** (emotional_statement_classifier_roadmap.md) — concept, kan op elk moment gebouwd worden, enkel afhankelijk van het al bestaande `raw_user_message`-event. Kern: een eigen, klein, begrensd classificatiemodel dat emotionele uitspraken herkent (bv. verdriet, frustratie, blijdschap) — draait PARALLEL aan microlearning.py (zelfde event, ander doel, geen overlap in data/logica). 12 fases uitgewerkt, fase 7-12 (uitbreidingen, incl. een mogelijk 5e insight-type voor Layer 7 en een can_interrupt()-koppeling via Layer 5) al goedgekeurd maar gemarkeerd als "kan wel, met architecturale afweging". Reactie blijft altijd een vooraf geschreven sjabloon (evt. met variatie) — nooit een vrij, gegenereerd invoelend antwoord, dat zou de grens richting LLM overschrijden.

*(Afgeronde werkpunten verplaatst naar `nova_changelog.md`, 18 juli 2026 — inclusief Personality pipeline deel 1+2, microlearning.py, Layer 2 opruimwerk, Layer 5 Fase 1-5, Layer 6-integratie response_style, emotion_engine decay, Layer 6 identity-blueprint-koppeling, het achtergrondthread-patroon, de `behavior_modifiers.py`-koppeling, en Activity Awareness Deel A. Aangevuld 9 augustus 2026 met punt 6b (`find_bridge()` gekoppeld aan gesprekslogica) en punt 24 (tombstone-eval, bug #32/#32/#32/#32/#36#36#36#36 opgelost) — zie changelog voor details van alle verplaatste punten.)*
...
- smart_home_roadmap.md — X-Sense rookmelders, Dreame-robotstofzuiger, Samsung Smart TV: elk een eigen, dunne module (geen Home Assistant nodig voor 3 merken). Concept, verre toekomst, geen prioriteit.
- personal_dates_roadmap.md — Persoonlijke datums (verjaardagen, examens, ...) laten onthouden en proactief herinneren, met bi-temporele opzet in het achterhoofd. Volledig uitgewerkt, nog niet gebouwd.
- date_calendar_roadmap.md — Datumrekenen, vaste/bewegende feestdagen, schoolvakanties, historische datums via Wikipedia's On This Day API + een "wil je erover lezen?"-keuzeflow die de pagina opent. Volledig uitgewerkt, nog niet gebouwd.
- layer1_word_embeddings_roadmap.md — Embeddings (Word2vec-achtig) als aanvulling op Layer 1's PMI, voor woorden die nooit letterlijk samen voorkwamen maar wel betekenisverwant zijn. Concept, open ontwerpvragen (zelf trainen vs. voorgetraind model).
- emotional_statement_classifier_roadmap.md — Apart, klein classificatiemodel dat emotionele uitspraken herkent, parallel aan microlearning.py. 12 fases uitgewerkt, kan op elk moment gebouwd worden.
- taal_en_redeneerlimieten_roadmap.md — 4 losse taalkundige ideeën (referentie-resolutie, ellipsis, case-based reasoning, hypothetisch redeneren) die nergens anders een thuis hadden. Idee 1+2 afgerond, idee 3+4 nog open (zie Volgende stappen, punt 8).
- **Vaag, nog niet uitgewerkt toekomstidee (15 augustus 2026):** Kevin herinnerde zich een idee waarbij `semantic.py`'s woordkennis ooit gebruikt zou worden om `response_engine.py`'s sjablonen zelf rijker/gevarieerder te laten formuleren (i.p.v. enkel vaste sjablonen invullen). **Gecontroleerd in nova_state.md en eerdere gesprekken: dit idee staat nergens exact zo vastgelegd** — het dichtstbijzijnde bestaande, wél uitgewerkte punt is werkpunt 5 hierboven (Layer 4-verrijking uitbreiden naar meer semantic-intents, inmiddels afgesloten) en punt 10 (Response Variant Learning, gewogen kiezen tussen vaste varianten — geen vrije generatie). Dit "semantic.py voedt sjabloonformulering"-idee is dus een nieuw, apart, ongetoetst idee, los van punt 6 hierboven (dat gaat over woordherkenning/lemmatisering bij het AANLEREN van concepten, niet over het GENEREREN van antwoordzinnen). Geen ontwerp, enkel vastgelegd zodat het niet verloren gaat. Belangrijke kanttekening vooraf: zodra dit richting "nieuwe zinnen vrij formuleren op basis van kennis" gaat i.p.v. vaste sjablonen invullen/kiezen, raakt dit al snel aan vrije taalgeneratie — expliciet te checken of dit nog puur symbolisch (grotere/slimmere sjabloon-keuze) blijft, of eigenlijk een ML/LLM-aanpak vereist, vóór hier ooit aan gebouwd wordt.
...
**Zesde feature op dit patroon, sinds 30 juli 2026: sjabloonvarianten gelijkgetrokken naar 5×5×3=75 voor alle vier insight-types.** Sinds de oorspronkelijke bouw (20-25 juli 2026) stonden `kennisdichtheid` (60 combinaties: 5 opening × 4 midden × 3 afsluiting) en `personality_drift` (36 combinaties: 4 opening × 3 midden × 3 afsluiting) achter op `woordverband`/`tijdspatroon` (beide al 75). Toen destijds bewust opengelaten als vraag, nooit beantwoord — nu alsnog rechtgetrokken: 1 nieuwe `midden`-variant voor `kennisdichtheid`, 1 nieuwe `opening` + 2 nieuwe `midden`-varianten voor `personality_drift`. Live bevestigd via 5x het `emergence`-debugcommando: alle nieuwe varianten kwamen door, plaatshouders (`{trait}`, `{aantal_shifts}`, `{concept}`, `{aantal_relaties}`) vulden correct in. **Bekend, bewust ongemoeid gelaten schoonheidsfoutje:** één nieuwe `personality_drift`-midden-variant ("...blijkt het meest bij te stellen zijn de laatste tijd...") klinkt grammaticaal iets krom — bewust niet meteen los hersteld, want een `midden`-fragment wordt via `.format()` gecombineerd met verschillende opening/afsluiting-varianten, en een geïsoleerde "verbetering" zou in een andere combinatie net een nieuw probleem kunnen veroorzaken; vraagt een rustige, aparte blik i.p.v. een snelle ingreep.

**Zevende feature op dit patroon, sinds 30 juli 2026: `_topic_naam_labels` aangevuld met `memory_query`.** Ontstaan doordat `detect_memory_query()` (punt 14, zie changelog) intussen gebouwd is en `_emit_topic("memory_query", bron="detect")` aanroept — een nieuw `topic_detected:`-onderwerp, dus volgens het vaste recept (zie "Vast recept, vervolgstap op bovenstaande" hieronder) overwogen voor Layer 7. Grondig geverifieerd i.p.v. aangenomen: de volledige keten `intent_router.py` (`_emit_topic`) → `memory.py` (`memory:interaction_added`) → `pattern_matcher.py`'s `detect_from()` (generieke `topic_detected:`-prefix-check, regel ~162) → `get_all_patterns()` → `analyze_timing_pattern()` (whitelist-check geldt enkel voor `activity_started:*`, dus `topic_detected:memory_query` stroomt ongehinderd door) is stap voor stap nagelopen in de echte broncode. Belangrijke conclusie, ook voor toekomstige, gelijkaardige gevallen: dit is GEEN nieuw insight-*type* — het is een nieuw onderwerp dat automatisch meegenomen wordt door het al bestaande `tijdspatroon`-insight (dezelfde generieke lens die ook "schaken"/"weer"/"coderen" al meeneemt). Een nieuw insight-type is enkel nodig bij een fundamenteel andere BEREKENING op de data (zoals `trending_topic` dat was) — een nieuwe naam die door een bestaande berekening mee opgepikt wordt, vereist enkel een label. Enige wijziging: 1 regel toegevoegd — `"memory_query": "vragen over mijn geheugen"`. Nog geen kandidaat in `analyze_timing_pattern()` op moment van testen (nog niet aan de `MIN_OBSERVATIES_VOOR_ANOMALIE`-drempel van 10), dus nog niet live bevestigd als daadwerkelijk gekozen tijdspatroon-insight — enkel dat de vertaling klaarstaat voor wanneer dat wel gebeurt.
...
`C:\Nova_AI\tests\`. Draaien vanuit de project-root: `pytest tests/ -v`. Stand 15 september 2026: **525 tests, allemaal groen** (~3s).
...
- **Layer 1** (`test_fase1.py`–`test_fase5.py`, 20 tests) — preprocessing, co-occurrence, PMI, queries, persistentie van `word_associations_learner.py`.
- **Layer 1, `find_bridge()`** (`test_find_bridge.py`, 8 tests) — brugwoorden tussen twee concepten (doorsnede van associaties, sortering, `top_k`, `min_confidence`, symmetrie). Zie ook de twee bullets hieronder voor de intent-koppeling erbovenop.
- **Layer 0** (`test_memory_fase4_archief.py` + `test_memory_archivering.py`, 9 tests) — `archive_old_events()`/`compress_ancient_events()` van `memory.py`, inclusief de volledige levenscyclus.
- **Layer 2** (`test_pattern_matcher.py`, 17 tests) — event-grouping, patroondetectie, anomaliedetectie (incl. het middernacht-uurverschil-randgeval).
- **Layer 5** (`test_randgeval_fase5.py`, 2 tests) — het score-combinatie-randgeval (coding + gebruikelijk moment tegelijk).
- **`weather.py`** (`test_weerwaarschuwing.py`, 18 tests, `@pytest.mark.parametrize`) — alle weerwaarschuwing-scenario's.
- **`semantic.py`** (`test_tombstone.py`, `test_reasoning.py`, `test_hard_delete_en_atomic_save.py`, 35 tests) — tombstone-mechanisme, reasoning-ketens (`is_a_chained`/`part_of_chained`/`causes_chained`), `find_contradictions()` zelf, hard-delete-guards, en `ConceptStore.save()`'s atomaire schrijflogica.
- **`semantic.py`, reasoning_engine_ideeen_roadmap.md's 6 ideeën** (`test_reasoning_engine_ideeen.py`, 33 tests) — `get_all_parts`, `related_to_chained` (incl. BFS-kortste-pad-regressietest), "waarom niet"-uitleg, part_of-cirkeldetectie + de bijbehorende `contradiction_checker.py`-meldingtekst, `compare_concepts`, `get_all_parts_with_property`.
- **`contradiction_checker.py`** (`test_contradiction_checker.py`, 10 tests) — spam-preventielogica van de tegenstrijdigheid-melder.
- **`intent_classifier.py`** (`test_intent_classifier.py`, 15 tests) — categorie-voorspelling (niet exacte confidence-scores), persistentie.
- **`intent_router.py`, reactivatie + woordherkenning** (`test_intent_router_reactivatie_en_woordmatch.py`, 9 tests) — de `-1F`-routingstap (`pending_reactivation` krijgt voorrang op de gewone ja/nee-confirm) en Bug #37 (woordherkenning bij "andere betekenissen van X").
- **`semantic.py` + `wikipedia_teacher.py`, reactivatie-flow** (`test_reactivatie_flow.py`, 10 tests) — `SemanticConceptsModule.teach()`/`handle_reactivation_confirm()` (de chat-vraag/antwoord-flow zelf) en `wikipedia_teacher.py`'s `_teach_word()`-guards (Bug #32/#36 en de vervolgbug).
- **`intent_router.py`/`chat.py`, `bridge_query`** (`test_bridge_query.py`, 17 tests + `test_bridge_query_integratie.py`, 11 tests, 9 augustus 2026) — `find_bridge()` gekoppeld aan een normaal gesprek (nova_state.md punt 6b). Eerste bestand test geïsoleerde kopieën van de regex-patronen en `on_bridge_query()`'s fallback-keten; tweede bestand bevestigt hetzelfde gedrag op de ECHTE `IntentRouter`/`ChatModule`-klassen, inclusief het end-to-end-pad via een synchrone nep-EventBus (`detect_bridge_query()` publiceert → `ChatModule.on_bridge_query()` ontvangt en verwerkt echt). Beide samen ontstonden uit een bewuste correctie: de eerste ronde testte enkel kopieën, wat als beperking benoemd werd; de tweede ronde vult dat aan.
- **Layer 1, `get_trending()`/`get_positive_words()`/`get_negative_words()`** (`test_trending_sentiment_lists.py`, 14 tests, 11 augustus 2026) — nieuwheid-gewogen trending-score, sentiment-lijsten met `min_score`-drempel (incl. de live-geconstateerde "woord in beide lijsten"-regressietest).
- **`intent_router.py`/`chat.py`, `trending_query`** (`test_trending_query.py`, 20 tests, 11 augustus 2026) — `get_trending()` gekoppeld aan een normaal gesprek (nova_state.md punt 6b, tweede deel). Zelfde soort geïsoleerde-kopieën-aanpak als de eerste `test_bridge_query.py`-ronde.
- **`emergence_engine.py`, Layer 7, `trending_topic`** (`test_emergence_trending_topic.py`, 16 tests, 11 augustus 2026) — 5e insight-type bovenop `get_trending()`. Bevat een volledige `reflect()`-integratietest met ALLE insight-types tegelijk (vaste procesregel sinds bug #22#22), inclusief dat de bestaande confidence-gate en timing-gate ook voor dit nieuwe insight-type correct werken.
- **`main.py`** (`test_main_typewriter_lock.py` + `test_main_achtergrond_en_response.py`, 12 tests, door Kevin zelf gebouwd rond Bug #30) — bewijst dat `_typewriter_lock` de typewriter-print echt blokkeert (geblokkeerde-thread-test, geen timing-gok) en dat twee gelijktijdige aanroepen niet door elkaar printen; dekt ook `on_chat_response()` (instant vs. typewriter-pad, prompt-herstel) en `achtergrond_loop()` (foutafscherming per module, de `aantal_loops % INTERVAL`-gates). Gecontroleerd en akkoord bevonden (8 aug 2026) — geen wijzigingen nodig, `import main` bleek veilig voor isolatie omdat alle bijeffecten achter `if __name__ == "__main__":` zitten.
- **`topic_suggestions.py`** (`test_topic_suggestions.py`, 25 tests, 13 augustus 2026) — punt 7, topic_events_roadmap.md Fase 5. `check_suggesties()` (patroon-check, timing-gate incl. het "nooit stiller dan voorheen"-principe bij ontbrekende `context_manager`, spam-preventie, whitelist, exceptie-afhandeling) en de nieuwe read-only `status_nu()` (t.b.v. het debug-commando `topic suggesties`, bewijst dat de state nooit gewijzigd of iets gepubliceerd wordt). `pattern_matcher`/`context_manager` als lichte stubs, geen echte zwaardere implementaties nodig voor deze isolatie.
- **`memory.py`, archief-uitbreiding** (`test_memory_archief_search.py`, 10 tests, 16 augustus 2026) — punt 14, deel 1. `search()`/`query()`/`get_stats()` doorzoeken/tellen sindsdien ook `interactions_old` (>90 dagen) via UNION ALL. Dekt: archief wordt standaard meegenomen, `include_archief=False` schakelt dit correct uit, recente + archief-resultaten worden samen gevonden zonder dubbels/gaten, sortering (`oldest_first`) blijft correct over beide tabellen heen, `get_stats()`'s nieuwe `totaal_events_recent`/`totaal_events_archief`-velden kloppen (incl. het randgeval van een leeg archief), en de bestaande `recent_weeks`-filter blijft correct samenwerken met de archief-logica.
- **`intent_router.py`/`chat.py`, `memory_query`** (`test_memory_query.py`, 30 tests, 16 augustus 2026) — punt 14, deel 2. Zelfde tweedelige aanpak als `bridge_query`/`trending_query` hierboven: geïsoleerde regex-patroon-tests (11, incl. negatieve gevallen tegen bridge/definitie/trending-zinnen om overlap uit te sluiten) + een nep-`ChatModule`-integratietest via nep-EventBus (19, incl. een regressietest voor het live-geconstateerde probleem dat `chat_response`-ruis zoals het help-menu als "gesprek" meetelde, en foutafhandeling bij ontbrekende/kapotte modules).
- **`emergence_engine.py`, gerichte topic-reactiviteit** (`test_emergence_topic_reactief.py`, 21 tests, 16 augustus 2026) — punt 15. Test tegen de ECHTE emergence_engine.py (niet een nagebouwde kopie), met `get_project_root()` gemonkeypatcht naar `tmp_path` voor volledige isolatie van de feedback-/cooldown-state-bestanden. Dekt `_check_topic_sterkte()` (5, incl. randgevallen), de cooldown-mechaniek (6, incl. het kernpunt dat verschillende topics elkaar niet blokkeren), het volledige `_on_elk_event()`-pad end-to-end (8, incl. fail-open-gedrag), en een kruistest die bevestigt dat `reflect()` en het gerichte pad dezelfde cooldown-state delen (2). Nep-EventBus in dit testbestand simuleert bewust ook het `"*"`-wildcardgedrag (zelfde patroon als de echte EventBus/memory.py) — een eerste testversie miste dat, wat 5 tests liet falen op een test-eigen fout, niet op de echte code.
- **`interruption_tracker.py`, Fase 6 tijdsvenster-verfijning** (`test_interruption_tracker_fase6.py`, 11 tests, 18 augustus 2026) — punt 7 (optioneel deel). Test tegen de ECHTE `InterruptionTracker` (`save_path` gemonkeypatcht naar `tmp_path` ná constructie, zelfde aanpak als bij modules met I/O in `__init__()`). Dekt backward-compatibiliteit (4 — oude aanroepen/opgeslagen data zonder `"vensters"`-sleutel blijven ongewijzigd werken), de venster-verfijning zelf (6 — vroeg/laat apart geteld, de venstergrens exact op 20.0 minuten als expliciete grenswaarde-test, venster-specifieke confidence, het kernscenario "activiteit-breed genoeg data maar één venster te dun" voor `has_enough_data()`, onbekende activiteit geeft nette `None`/`False`), en persistentie (1 — venster-data overleeft een save/load-cyclus, simuleert een `/reboot`).
- **`variant_feedback_logger.py`, Response Variant Learning Fase 1+2** (`test_variant_feedback_logger.py`, 13 tests, 20 augustus 2026). Nep-EventBus (`subscribe`/`publish`), `data_dir` als `tmp_path`. Dekt Fase 1-logging (event → correcte regel in `variant_feedback.jsonl`), Fase 2-sentiment-koppeling (eerstvolgende reactie correct gekoppeld; **counter-test** bevestigt dat zonder geladen `sentiment_classifier.model` er nooit gegokt wordt op "positief" — `classificeer()` wordt dan gewoonweg niet aangeroepen), precies-1-open-koppeling-per-sjabloon (twee variant-keuzes zonder tussenliggende reactie: enkel de laatste krijgt de koppeling), onafhankelijkheid tussen verschillende sjablonen, de `MIN_OBSERVATIES_PER_VARIANT`-drempel (geen gewichten eronder, wel erboven), de `MIN_GEWICHT`-garantie (nooit 0, ook niet bij uitsluitend negatieve reacties), het neutrale gewicht voor data-loze varianten, en het correct herbouwen van de in-memory scores uit een bestaand bestand bij een herstart (incl. een beschadigde regel die het inlezen niet mag blokkeren).
- **`variant_kiezer.py`, gedeelde kies-functie voor Response Variant Learning** (`test_variant_kiezer.py`, 9 tests, 20 augustus 2026). Dekt gelijke-kansen-gedrag zonder logger, dat het `variant_gekozen`-event altijd gepubliceerd wordt met de juiste velden, gewogen keuze bij een beschikbare logger, dat een fout in `get_gewichten()` nooit crasht (valt terug op gelijke kansen), en de nieuwe `uitsluiten_indices`-parameter: **counter-tests** bevestigen dat een uitgesloten index nooit gekozen wordt (ook niet met het hoogste gewicht) én dat de variant-lijst zelf nooit gefilterd wordt (index-betekenis blijft stabiel, nodig omdat `get_gewichten()`'s scores anders door elkaar zouden husselen).
- **`microlearning.py`, koppeling met `emotion_engine.py`** (`test_microlearning_emotion_koppeling.py`, 11 tests, 22 augustus 2026) — voorheen punt 9 ("Emotie-triggers, momenteel maar 1 trigger-bron"). Test een geïsoleerde kopie van de nieuwe `_SIGNAAL_NAAR_EMOTION_TRIGGER`-mapping/koppelmethode (`DummyEventBus`/`DummyPersonalityEngine`/`DummyEmotionEngine`, GEEN echte `MicroLearning`-instantie — die heeft I/O in `__init__()` voor 6 bestanden, buiten scope van deze eerste testronde). Dekt alle 6 signaal→trigger-koppelingen (`frustratie`/`interesse`/`verwarring`/`focus`/`waardering`/`kilte`), dat een onbekend signaal (`neutraal`) geen trigger aanroept, dat ontbrekende `personality`/`emotion`-modules nooit crashen (3 randgevallen), en dat een interne fout in `emotion.apply_trigger()` zelf ook nooit doorsijpelt.
- **`emotion_engine.py`, `last_trigger`-bugfix** (`test_emotion_engine_last_trigger.py`, 3 tests, 22 augustus 2026) — regressietest voor een bug die tijdens het live-testen van bovenstaande koppeling aan het licht kwam: `apply_trigger()` werkte `state["last_trigger"]` nooit bij (enkel `last_reaction` wel), wat nooit opviel zolang `"excitement"` de enige trigger-bron was. Test tegen de ECHTE `EmotionEngine` (`__new__()` om `__init__()`'s bestands-I/O te omzeilen, `rules`/`state` handmatig opgebouwd — zelfde patroon als `ContradictionChecker`/`PatternMatcher`). Bevestigd rood-zonder-fix/groen-met-fix specifiek voor deze bug. Dekt: `last_trigger` volgt de meest recente van meerdere achtereenvolgende, verschillende triggers (3), `last_reaction`-gedrag blijft ongewijzigd correct naast de fix (2), en een onbekende trigger (niet in `mood_shifts`) wijzigt `last_trigger` niet (vroege return blijft intact).
- **`last_context.py`** (`test_last_context.py`, 25 tests, 22 augustus 2026) — Taal & Redeneerlimieten, idee 1+2. Test tegen de ECHTE `LastContext`, geen monkeypatch nodig (geen I/O in `__init__()`), eigen `NepKlok`-helper i.p.v. echte `sleep()`. Dekt basisgedrag, de veiligheidsklep (nieuw concept overschrijft altijd, ook binnen de vervaltijd), vangnet-verval (incl. de exacte grenswaarde), `ververs_timestamp()` (incl. een simulatie van een hele reeks vervolgvragen), `clear()`, `init_module()`.
- **`intent_router.py`, referentie-resolutie + ellipsis** (`test_referentie_resolutie.py`, 55 tests, 22 augustus 2026) — Taal & Redeneerlimieten, idee 1+2, vervolg. Test tegen de ECHTE `IntentRouter` + `LastContext`, nep-EventBus. Dekt `_is_kale_verwijzing()`, `_herschrijf_met_concept()`, alle drie paden van `_verwerk_referentie()` (incl. een counter-test dat een geslaagde resolutie de klok verlengt zonder de inhoud te wijzigen), `verwerk_referentie_antwoord()`, `_emit_topic(concept=...)`, de 7 tabel-gedreven detect-functies (`detect_relation_check`/`detect_part_of_check`/`detect_subtypes_query`/`detect_parts_query`/`detect_related_to_check`/`detect_compare_concepts`/`detect_bridge_query`, bevestigen de vaste eerst-genoemde-term-regel), de tabel-lus-reset, en 5 end-to-end-scenario's door de echte `route()` heen. Zie `nova_changelog.md` voor het volledige bouwverslag en de live-testresultaten.
- **`emergence_engine.py`, Layer 7, `scherm_focus`** (`test_emergence_screen_focus.py`, 16 tests, 15 september 2026) — punt 17. `analyze_screen_focus()`/`_formuleer_scherm_focus()`: alle voorwaarden die geen insight mogen opleveren (ontbrekende/falende `context_manager`, lege/`None` `screen_focus`, niet-gewhitelist activiteit, te kort bezig — incl. de exacte-drempel-grens), het gelukkige pad, en de integratie met `analyze_meta_patterns()`/`LAYER4_DREMPELS`. Bevat 2 expliciete regressietests tegen de live-testbug (zie changelog): pattern_matcher-namen (`coderen`/`coding_gedetecteerd`) mogen nooit via de nieuwe `SCHERM_FOCUS_ACTIVITEITEN`-whitelist komen, en die whitelist moet een eigen, apart object blijven t.o.v. `INSIGHT_WAARDIGE_ACTIVITEITEN`. `get_project_root()` gemonkeypatcht naar `tmp_path`, zelfde aanpak als `test_emergence_topic_reactief.py`.
- **`topic_suggestions.py`, activiteit-trigger** (`test_topic_suggestions_activiteit.py`, 19 tests, 15 september 2026) — punt 18. `_tijd_kandidaten()`/`_activiteit_kandidaten()`/`check_suggesties()`: elke trigger apart, beide triggers samen (geen dubbele publicatie), whitelist-filtering van `get_relevant_topics()`-resultaten, `can_interrupt()` als gedeelde harde gate voor beide triggers, spam-preventie die werkt ongeacht welke trigger het topic vond, en een bevestiging dat de bestaande `status_nu()` ongewijzigd blijft werken. `get_project_root()` gemonkeypatcht naar `tmp_path`, zelfde aanpak als `test_topic_suggestions.py`.
- **`calendar.py`, nieuwe kalendermodule** (`test_calendar.py`, 28 tests, 16 september 2026). Test tegen de ECHTE `CalendarModule` (echte package-import, `from modules.time.calendar import ...`, zelfde patroon als `test_fase1.py`), nep-EventBus + nep-`zone`-module met een vaste, injecteerbare datum (geen afhankelijkheid van de werkelijke "vandaag", zelfde principe als `test_last_context.py`'s `NepKlok`). Geen monkeypatch nodig — geen I/O in `__init__()`. Dekt `init_module()`/registratie (4), de `zone`-koppeling en de fallback op `datetime.now()` zonder zone (3), alle 5 gerichte antwoorden + het algemene overzicht + de fallback op "algemeen" bij een onbekend/ontbrekend `type` (9), directe methode-aanroepen los van het event-pad (2), randgevallen over verschillende datums — jaarwisseling, ISO-week 53, enkelvoudig dagcijfer zonder leidende nul, alle 12 maanden, alle 7 dagen (7, deels geparametriseerd), en een consistentie-check tussen `DAGNAMEN`/`MAANDNAMEN` en Python's eigen `weekday()`-nummering om een stille off-by-one te voorkomen (3).
- **`calendar.py`, datumrekenen-uitbreiding** (`test_calendar_datumrekenen.py`, 47 tests, 17 september 2026) — date_calendar_roadmap.md Onderdeel 1. Zelfde nep-EventBus/nep-`zone`-aanpak als `test_calendar.py`, vaste referentiedatum donderdag 17 september 2026. Dekt `_parse_datum()` per herkend patroon apart — morgen/overmorgen/vandaag incl. de "overmorgen bevat morgen als substring"-volgordetest (4), weekdagnamen incl. het "vandaag zelf" → volgende week-randgeval (3), "over N dagen/weken" incl. enkelvoud en nul (5), cijfernotatie incl. ongeldige datums en 29 februari in een niet-schrikkeljaar (7), dag+maandnaam incl. de automatische jaarsprong, het "vandaag zelf blijft dit jaar"-grensgeval, en alle 12 maandnamen (7), en onherkende tekst waaronder de bewuste "kerst wordt nog niet herkend"-scope-test (2) — plus `dagen_tot()`/`dag_van_de_week()`/`datum_plus()` los getest als pure rekenfuncties, en elk van de drie via `on_calendar_intent()` inclusief de nette tegenvraag bij een onherkende/ongeldige datum en de "geleden"-formulering bij een datum in het verleden. Live bevestigd voor alle vijf zinsvormen plus de bewuste "hoeveel dagen tot kerst" → tegenvraag.

**Vaste werkwijze, zie ook het "Werkafspraak"-kopje bovenaan dit bestand:** isolatie via `tmp_path`, nooit de echte `data/`-bestanden; eerst checken OF/WANNEER een module I/O doet in `__init__()` vóór een fixture geschreven wordt (`ContradictionChecker`/`PatternMatcher` hebben geen injecteerbaar pad en vereisten `monkeypatch`, in tegenstelling tot `MemoryModule`/`WordAssociationsLearner`/`ConceptStore`). `IntentRouter`/`ChatModule` blijken ook injecteerbaar zonder monkeypatch — beide doen geen I/O in `__init__()`, een nep-EventBus (`publish`/`subscribe`/`.modules`-dict) volstaat.

**Nog open, bewust niet gedekt:** `_check_missing_events()` (Fase 3 Deel B van `pattern_matcher.py`) en `predict_next_occurrence()` — beide sterk tijdgebonden, zouden `datetime.now()` moeten mocken; `semantic.py`'s sense-splitsing/disambiguatie-details en synoniem-toevoeging-varianten; `math.py`, `chess_engine.py` (leunt op extern .exe-proces), en de meeste Layer 4/6/7-modules.

**`intent_router.py` (~4000 regels) en `chat.py`: BEWUST GEEN volledige dekking, expliciet benoemd om een te optimistische indruk te voorkomen.** Getest zijn tot nu toe de stukken die tijdens een sessie zelf gebouwd/gewijzigd werden: `bridge_query` (zie hierboven), de `-1F`-reactivatie-routing, Bug #37's woordherkenning, en (22 augustus 2026) referentie-resolutie/ellipsis. **Correctie op eerdere versie van deze alinea:** `detect_relation_check`, `detect_part_of_check`, `detect_subtypes_query`, `detect_parts_query`, `detect_related_to_check`, `detect_compare_concepts` en `detect_bridge_query` stonden hier voorheen als "NIET getest" vermeld — sinds de referentie-resolutie-uitbreiding (`test_referentie_resolutie.py`) hebben deze 7 methodes WEL testdekking, maar uitsluitend gericht op hun nieuwe `_laatste_concept_kandidaat`-gedrag (welke term ze doorgeven aan `last_context`), NIET op hun volledige regex-patroonherkenning zelf (de verschillende triggerzin-varianten per functie blijven ongetest). Nog steeds volledig NIET getest: `detect_activity`, `detect_preference`/`detect_preference_query`, classifier-fallback-pad, chess/math/weather/time-routing, help/uitleg-routing, en de meeste `on_*()`-handlers in `chat.py` (`on_definition`, `on_relation_check`, `on_part_of_check`, `on_subtypes_query`, `on_parts_query`, `on_related_to_check`, `on_compare_concepts`, `on_parts_with_property`, `on_related_to`, `on_synonym`, `on_antonym`, `on_used_for`, `on_causes`, `on_properties`, `on_meaning`, `on_concept_learned`, `on_wiki_response`, `on_identity_question`, `on_self_architecture_question`). Risico-inschatting nog niet gemaakt — als dit ooit opgepakt wordt, eerst afwegen welke van deze methodes het hoogste stille-regressierisico hebben (zelfde aanpak als de rest van dit hoofdstuk).
...
- **Vast recept, vervolgstap op bovenstaande (sinds 30 juli 2026):** elke nieuwe module/uitbreiding die een nieuw `activity_started:<naam>`- of `topic_detected:<naam>`-event introduceert, moet expliciet overwogen worden voor `emergence_engine.py`'s Layer 7-whitelist. Voor `activity_started:*`: nieuwe activiteiten zijn standaard UITGESLOTEN van insight-generatie (zie `INSIGHT_WAARDIGE_ACTIVITEITEN`, whitelist-principe) — enkel bewust toevoegen als Kevin akkoord is dat Nova hier ooit spontaan over mag praten, en dan ook meteen een leesbaar label toevoegen aan `_activity_naam_labels`. Voor `topic_detected:*`: mag al generiek door de whitelist-check, maar verdient wel een vertaling in `_topic_naam_labels` als de interne naam Engels/technisch is (anders klinkt een Nederlandse sjabloonzin met een los Engels woord erin). Zonder deze stap loopt Layer 7 het risico ofwel stil te blijven over relevante nieuwe activiteiten (whitelist vergeten), ofwel — erger — ooit een ongewenste/te persoonlijke activiteit alsnog te noemen als de whitelist-check zelf ooit per ongeluk versoepeld wordt. Ontdekt tijdens een volledige controle van `patterns_layer2.json` (zie changelog #26) toen bleek dat `activity_started:unknown_gedetecteerd` (total=67) ongemerkt de sterkste kandidaat was geworden.
