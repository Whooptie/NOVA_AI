...
| math_uitleg.py | ✅ Klaar (5 aug 2026) | Uitgebreide, vaste uitlegteksten per math-functie ("leg uit X"/"wat is X"/"hoe werkt X"). 13 canonieke teksten, 26 gedekte functienamen. Zie eigen sectie hieronder voor het volledige verslag. |
...
| 30 | Typewriter-effect + achtergrondthread: tekst raakt zichtbaar door elkaar gehusseld op tekenniveau bij lange antwoorden (ontdekt tijdens het testen van `math_uitleg.py`, 5 aug 2026 — zie "Volgende stappen" punt 23 voor het volledige verslag). Vermoedelijk een race condition tussen het typewriter-effect en een gelijktijdige achtergrondthread-publicatie (session_watcher/emergence_engine/weather via `achtergrond_loop()`). | main.py (vermoedelijk, nog niet bevestigd) | 🔴 Hoog | 🔲 Open — nog te onderzoeken of `wachten_op_input` enkel de prompt beschermt maar een al-lopend typewriter-effect niet onderbreekt, en of een lock/queue rond het printen dit oplost. |
...
| 32 | `add_sense()`'s deduplicatie-loop (regel 188-209 van `semantic.py`) vergelijkt enkel op `definition`-tekst, zonder te filteren op `status`. Een sense met `status == "rejected"` komt daardoor STILZWIJGEND terug als `"confirmed"` zodra dezelfde definitie-tekst later opnieuw wordt aangeboden met `source="user"` (bv. via een nieuwe Wikipedia-match of hernieuwde chat-input) — de dedup-check matcht de bestaande rejected sense i.p.v. een nieuwe aan te maken, zonder enige waarschuwing dat dit een eerder afgewezen feit was. Bevestigd via `test_tombstone.py` (8 aug 2026) — beantwoordt de open vraag uit het voormalige punt 25 van "Volgende stappen". `add_relation()`'s eigen dedup-check heeft dit lek NIET (faalt gewoon stil zonder de status van een rejected relatie te wijzigen). | semantic.py (`SenseEngine.add_sense()`) | 🟡 Midden | 🔲 Open — bewust nog niet gefixt (Kevin's keuze, 8 aug 2026). Mogelijke fix: dedup-check een `rejected`-match laten overslaan, zodat de code doorvalt naar "nieuwe sense aanmaken" i.p.v. de oude te doen herleven. Nog te beslissen: moet een bewuste re-bevestiging door Kevin zelf (`source="user"`) een uitzondering blijven, of moet zelfs dat via een expliciete un-reject-actie lopen? |
...
18. 🔴 **Typewriter-effect + achtergrondthread: door elkaar gehusselde tekst bij lange antwoorden** — ontdekt tijdens het live testen van de uitlegmodule (`math_uitleg.py`, zie de eigen sectie "✅ math_uitleg.py" verderop, 5 aug 2026). Bij `leg me uit runge kutta` verscheen een deel van Nova's antwoord zichtbaar gehusseld op tekenniveau (bv. "Waarom dit wiskundig klopt:Nova: W adcohotr, 4j ek ebeern..." — twee stukken tekst die letter-voor-letter door elkaar geregen zijn). Dit gebeurde niet bij math_uitleg.py's andere, even lange antwoorden in dezelfde testsessie (newton, dijkstra) — vermoedelijk een race condition tussen het bestaande typewriter-effect (het geleidelijk uittikken van een antwoord, zie het "verse prompt"-mechanisme/`wachten_op_input` hierboven) en een gelijktijdige achtergrondthread-publicatie (`achtergrond_loop()` in `main.py`, bv. session_watcher/emergence_engine/weather) die ergens halverwege het uittikken een eigen bericht print. Math_uitleg.py's teksten (2600-2900 tekens) maken dit zichtbaarder dan voorheen simpelweg omdat het typewriter-effect nu veel langer duurt per antwoord, waardoor de kans op een overlappende achtergrondpublicatie toeneemt — het onderliggende probleem zit dus niet in math_uitleg.py zelf, maar in hoe het typewriter-effect en `on_chat_response()`/proactieve publicaties elkaar kunnen overlappen. Nog te onderzoeken: of het bestaande `wachten_op_input`-vlagmechanisme enkel de PROMPT beschermt (zoals ontworpen) maar niet een al-lopend typewriter-effect zelf onderbreekt/pauzeert, en of een simpele lock/queue rond het printen van elk antwoord dit zou oplossen.
19. 🟢 **Natuurlijke-taal-naar-SymPy-vertaler voor math** — ontstaan uit de nieuwe `math`-classifier-koppeling (5 augustus 2026, zie `nova_changelog.md`). Die kale koppeling (`{"expr": text}`, zelfde patroon als weather/time) dekt enkel zinnen die de classifier op taalgebruik als `math` herkent maar die al symbolische syntax bevatten. Concreet bevestigd met een test dat dit NIET voldoende is voor natuurlijke taal: `sp.sympify("wat is drie plus vijf")` faalt met een `SyntaxError` — SymPy begrijpt geen woorden zoals "drie"/"plus", enkel echte syntax (`3+5`). Zou een nieuw stuk werk vereisen: getallen-in-woorden herkennen ("drie" → 3), operatiewoorden naar symbolen vertalen ("plus"/"min"/"maal"/"gedeeld door" → +/-/*//), en zinsstructuren als "zoek de waarde van X, vergelijking ..." omzetten naar de juiste `solve_sym(...)`-aanroep. Nog geen ontwerp, enkel als idee vastgelegd — Kevin's woorden: "eerst optie 1 en maak dan een nieuw puntje voor de volgende stappen lijst voor die nlp".
20. 🟡 detect_memory_query() — reactieve toegang tot memory.py's Query API vanuit een gewoon gesprek — memory.py's Fase 3 (search(), query(), get_stats(), find_similar()) bestaat en werkt volledig, maar er is GEEN detect_*() in intent_router.py die een vraag zoals "wat heb ik je al eens gevraagd over X?" of "wat komt vaak terug in onze gesprekken?" herkent en doorstuurt naar die functies. Precies hetzelfde soort gat als find_contradictions() (extern artikel, zie punt 6): een krachtig, werkend mechanisme zonder aanroeper vanuit het gesprek zelf. De enige huidige toegang loopt via debug-commando's ("geheugen stats", "geheugen gezondheid") die bedoeld zijn als technische debug-info, niet als natuurlijk antwoord op een vraag.
    ⚠️ **Bevestigd gat ontdekt (6 augustus 2026, via broncode-controle van memory.py):** `search()`/`query()` doorzoeken UITSLUITEND de `interactions`-hoofdtabel (< 90 dagen). `interactions_old` (90 dagen-1 jaar, regel 570-600) en het gecomprimeerde `data/archive/interactions_compressed_*.jsonl.gz`-archief (1+ jaar, regel 604-643) worden door geen van beide functies doorzocht — de data bestaat wel, maar is onbereikbaar via de huidige Query API. Voor #20 om echt over de VOLLEDIGE geschiedenis te werken, moeten `search()`/`query()` eerst uitgebreid worden met een extra doorzoeking van `interactions_old` (nog gewoon SQL, UNION-achtig) en een apart, tragere fallback-pad voor het `.jsonl.gz`-archief (`gzip.open()` + lezen, geen SQL-index mogelijk).
20b. 🟢 **Proactieve datum-terugkoppeling ("vorig jaar op deze datum...")** — nieuw idee, nog niet uitgewerkt. Ander soort feature dan #20 hierboven: niet reactief (Kevin vraagt), maar PROACTIEF — Nova zoekt ongevraagd in interactions.db (+ archief, zie #20's gat hierboven) naar events rond dezelfde kalenderdatum in eerdere jaren, en meldt een sterke overeenkomst ongevraagd (bv. "vorig jaar op deze datum hadden we het ook over X"). Zelfde familie als #9 (topic events) en #21 (gespreks-contextuele proactiviteit) — "iets dat al bekend is, ongevraagd terugkoppelen op het juiste moment", maar dan datum-getriggerd i.p.v. onderwerp-getriggerd. Zou vermoedelijk als een extra periodieke check in achtergrond_loop() passen (zelfde patroon als emergence_engine.reflect()), met een eigen can_interrupt()-check via Layer 5, en hangt af van personal_dates_roadmap.md/date_calendar_roadmap.md voor het datum-vergelijkingsstuk. Nog te ontwerpen: welke confidence-drempel "sterk genoeg" is om dit te melden, en of dit ENKEL over interactions.db gaat of ook events uit Layer 2 (patterns_layer2.json) zou moeten meenemen.
21. 🟢 Gespreks-contextuele proactiviteit i.p.v. enkel tijd-gebaseerd (Layer 7) — reflect() (emergence_engine.py) is sinds 28 juli 2026 proactief, maar uitsluitend via een vaste klok (EMERGENCE_CHECK_ INTERVAL_MINUTEN = 10) die periodiek het sterkste patroon over de HELE geschiedenis checkt, volledig los van het actuele gespreksonderwerp. Als Kevin NU over een specifiek, vaak terugkomend onderwerp praat, reageert Nova daar niet direct op — enkel toevallig, als de 10-minutenklok net op dat moment afloopt met exact dat onderwerp als sterkste bevinding. Idee: bij een herkend topic_detected: < naam > -event (bestaat al, Fase 1-4 van topic_events_roadmap.md) een korte check laten lopen of Layer 1/2 al iets sterks weet over PRECIES dat onderwerp, i.p.v. te wachten op de generieke klok. Hangt samen met punt 24 — beide draaien om "iets terugkoppelen dat al bekend is", één reactief (Kevin vraagt actief), één proactief (Nova herkent het onderwerp terwijl het speelt).
22. 🟢 **README-claim "no internet" verduidelijken** — extern advies (brede architectuurreview, 6 aug 2026) merkte op dat de README's claim "no internet" niet klopt met de werkelijkheid: Wikipedia-teacher, weather.py en de ipinfo.io-locatiecheck gebruiken wel degelijk netwerkverbindingen. De kernbelofte van Nova (geen cloud-AI, geen LLM-inference extern) blijft overeind — enkel de formulering is misleidend voor wie de README letterlijk leest. Idee: "no internet" vervangen door iets als "geen cloud AI / geen externe AI-inference — enkele modules (Wikipedia, weer, locatie) gebruiken gewone, niet-AI netwerkverbindingen." Kleine tekstwijziging, geen codewijziging.
...
24. 🟢 **Cognitive coupling — architectuurfilosofie voor later, geen taak** — zelfde externe review noemt "cognitive coupling" (steeds meer modules die intern weten hoe andere modules werken, bv. response_engine → loaded_modules, context_manager/emergence → meerdere specifieke layers) als Nova's belangrijkste architecturale risico op langere termijn — niet performance, niet Python, niet SQLite. Nog volledig beheersbaar bij de huidige ~100 modules. Idee voor de verre toekomst (geen ontwerp, enkel vastgelegd zodat het niet vergeten wordt): een centrale "cognitive API"/capability registry, zodat modules met elkaar praten via een vast contract in plaats van elkaars interne structuur rechtstreeks te kennen — relevant zodra Nova richting honderden modules groeit, niet nu.
25. 🟡 **Automated eval voor het tombstone-mechanisme** — extern advies (gecombineerde review, 6 aug 2026) stelde voor om specifiek te testen of `status == "rejected"` intact blijft na toekomstige code-wijzigingen of nieuwe Wikipedia/auto-extract-zoekacties — dus dat een afgewezen feit nooit stilzwijgend terugkomt. `scripts/test_atomic_save.py` bestaat al als voorbeeld-patroon (los testscript, draait op een tijdelijke kopie van `concepts.json`, nooit het origineel) maar test iets anders (crash-bestendigheid van `save()`). Idee: een vergelijkbaar, apart testscript of een `assert`-gebaseerde test (zie ook punt 28, testsuite-zwakte) die het scenario uit `nova_changelog.md` naspeelt — `hond is_a meubel` → weerleg → contradicties weg → probeer opnieuw dezelfde relatie toe te voegen via `add_relation`/Wikipedia-achtige extractie → bevestig dat de dedup-logica in `add_sense`/`add_relation` de rejected-rij herkent en niet stilzwijgend laat herleven. **Update (8 aug 2026):** test gebouwd (`test_tombstone.py`, 6 tests, allemaal groen). De test bevestigde de vermoede zwakte meteen — zie bug #32 in de bugtabel bovenaan voor het openstaande dedup-lek in `add_sense()` (reasoning-laag zelf, `get_relations()`/`part_of_chained()`/etc., bleek wél overal correct `rejected` te negeren).
26. 🟢 **Debug-commando `audit <woord>` — audit-log doorzoeken/filteren** — extern advies stelde een aparte inspectie-CLI voor om `concepts.json`/audit-logs te doorzoeken zonder handmatig het JSON-bestand te doorspitten. Bewust NIET als los CLI-script gebouwd — dat zou een tweede toegangsweg naast de chat betekenen, wat ingaat tegen Nova's kernprincipe dat alles via het gesprek zelf loopt (en tegen de "cognitive coupling"-voorzichtigheid van punt 30: twee plekken die dezelfde data tonen). De bestaande inspectiemiddelen (`concept_overview.py` voor "wat weet je over X", `debug_commands.py` voor `geheugen stats`/`contradicties`/enz.) dekken al veel, maar geen ervan laat je specifiek filteren op audit-log-inhoud (bv. "toon alle relaties afgewezen deze maand", "toon alle concepten met source=wikipedia"). Idee: dit als extra debug-commando toevoegen aan de bestaande `debug_commands.py`-dispatch (zelfde patroon als de andere 17 commando's), niet als apart script.
